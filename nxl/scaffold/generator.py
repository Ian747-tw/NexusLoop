...
- Directories: .nxl/, logs/, skills/, dashboard/
"""
...
class OnboardingResult:
    ...
    onboarding is skipped (``nxl init --auto``).
    """
...
class ScaffoldGenerator:
    ...
    def generate_config_dir(self) -> None:
        """Create .nxl/ and write all YAML / JSON config files."""
        config_dir = self.project_dir / ".nxl"
    ...
    def _render_hardware_yaml(self) -> str:
        ...
        if ob.hardware_summary:
            return (
                "# NexusLoop — hardware configuration\n"
                "# Auto-detected during `nxl init`.\n"
                "# Run `nxl doctor` to refresh.\n\n"
                + ob.hardware_summary
            )
...
# NexusLoop Policy Configuration
# Generated: {timestamp}
...
# ---------------------------------------------------------------------------
# .nxl/hardware.yaml (default when no detection data available)
# ---------------------------------------------------------------------------

_HARDWARE_YAML_DEFAULT = """\
# NexusLoop — hardware configuration
# Auto-populated by `nxl doctor`.
# Run `nxl doctor` to detect and fill real values.
...
# NexusLoop — Python environment configuration
...
# ---------------------------------------------------------------------------
# .nxl/permissions.yaml
# ---------------------------------------------------------------------------

_PERMISSIONS_YAML_TEMPLATE = """\
# NexusLoop — agent permissions
# Controls what autonomous agents are allowed to do.
# These values are enforced by the PolicyEngine (nxl check).
...
Violations are enforced by `nxl check`.  Critical violations
...
- Run `nxl status` — check current phase and queue.
...
- If the change touches reward logic: run `nxl check
  --action edit_reward` first.
- If the change touches eval logic: run `nxl check
...
- If stuck (N consecutive discards without improvement), trigger
  `nxl research` to do a literature refresh.
...
- `.nxl/state.json` — update via `nxl` CLI
- `skills/*.md` — add new skills or improve existing ones

### Files you MAY modify with `nxl check` approval:
- Reward function code — requires `edit_reward` gate
- Evaluation code — requires `edit_eval` gate
- `.nxl/policy.yaml` — requires `modify_policy_yaml` gate
- `.nxl/permissions.yaml` — requires human approval

### Files you must NEVER modify:
- `NON_NEGOTIABLE_RULES.md`
- `CLAUDE.md` (this file)
- `AGENT.md`
- `.nxl/policy.yaml` without `check` approval
...
- **state.json**: Managed by the `nxl` CLI.  Edit via CLI.
...
- **Never silently change eval logic.**  Any change to the eval
  ...
  approved via `nxl check --action edit_eval`, and
...
- **Never silently change reward logic.**  Any change to reward
  ...
  approved via `nxl check --action edit_reward`.
...
- **Never install packages outside policy.**  Run
  `nxl check --action global_install` before any
...
This document describes how the NexusLoop orchestrator makes
...
The orchestrator triggers `nxl research` (literature refresh)
...
- Human runs `nxl research` manually.
...
```
nxl check --action <action_name> --details <json>
```
...
initialized_at            When `nxl init` was run.
...
Call `nxl check --action <action>` before any gated action.
...
- Approval via `nxl check --action edit_reward` is required
