...
You are the DRL research agent. Use the NexusLoop controller runtime.
...
3. **Check state**: `nxl status --project-dir .`
...
```bash
nxl run --project-dir . --once
nxl run --project-dir . --agent-backend claude
nxl run --project-dir . --agent-backend codex
```
...
- live loop status is written to `.nxl/state.json` and shown in the dashboard
- autonomous execution requires permissive onboarding policy such as `open`
- risky actions must go through `nxl check`
...
For runs > 4 hours:
- Start dashboard first: `nxl dashboard --project-dir . &`
