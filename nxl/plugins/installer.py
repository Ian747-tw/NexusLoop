...
Called by ``nxl init --plugin [cc|codex|both]``.
"""
...
def install_skill_generator_backend(project_dir: Path, context: str) -> bool:
    ...
    dst = project_dir / ".nxl" / "backend" / "skill_generator.md"
    ...
    console("Installed: .nxl/backend/skill_generator.md", "success")
    return True


def remove_managed_plugin_files(project_dir: Path) -> list[Path]:
    """Remove plugin files managed by NexusLoop from the target project."""
    project_dir = Path(project_dir).resolve()
    removed: list[Path] = []
