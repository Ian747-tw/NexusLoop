...
`nxl` is an autonomous DRL research loop installed globally on this
...
```bash
nxl doctor              # check environment (run first)
nxl status              # current phase, best run, counters
nxl plan [--refresh]    # ranked experiment plan
nxl run [--dry-run]     # start the continuous agent loop
nxl run --once          # run one autonomous agent cycle
nxl run --agent-backend codex
nxl check --action X    # gate any action against hard rules
nxl research            # trigger mid-training research refresh
nxl dashboard --port 8765 &   # start live dashboard
```

## Mandatory Pre-Flight (run before anything else)

```bash
nxl doctor              # must show 0 failures
nxl status              # read current phase + best run
```

Read `NON_NEGOTIABLE_RULES.md`. Every rule is enforced programmatically — do not
assume you can skip a rule check.

If present, read `.nxl/spec_compact.md` before planning changes.
...
```bash
nxl check --action <type> --details '{"description": "..."}'
```
...
`nxl run` is the controller. It keeps selecting the next
...
- The live loop state is written into `.nxl/state.json` and exposed in the dashboard.

Use `nxl run --once` if you want one autonomous cycle only.

## Required Backbone APIs

During autonomous cycles, use the project helpers instead of raw file edits:

- `nxl.logging.registry.ExperimentRegistry`
- `nxl.logging.journal.ProjectJournal`
- `nxl.logging.incidents.IncidentLog`
- `nxl.logging.handoffs.HandoffLog`
...
```python
from nxl.core.agent_contract import record_skill_consultation
...
Use Python's `nxl.logging.registry.ExperimentRegistry` for safe
atomic writes:
