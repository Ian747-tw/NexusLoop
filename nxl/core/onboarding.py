"""
onboarding.py — Interactive onboarding flow for NexusLoop.

Uses stdin/stdout only (no external TUI libraries).
Config is persisted under .nxl/ inside the target project.
...
from nxl.core.hardware import HardwareDetector, HardwareInfo
...
class OnboardingFlow:
    """
    Interactive onboarding wizard for a NexusLoop project.

    Parameters
    ----------
    project_dir : Path
        Root of the target project (where .nxl/ will be written).
    auto : bool
    ...
    def run(self) -> OnboardingResult:
        ...
        print()
        print(SEP)
        print("  NexusLoop — Project Onboarding")
        print(SEP)
        if self.auto:
...
""")
    ...
    def save_results(self, result: OnboardingResult) -> None:
        """Persist OnboardingResult to .nxl/onboarding.yaml (or .json)."""
        config_dir = self.project_dir / CONFIG_DIR
        config_dir.mkdir(parents=True, exist_ok=True)
