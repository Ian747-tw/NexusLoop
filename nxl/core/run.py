...
from nxl.core.agent_contract import initialize_contract, load_contract
from nxl.core.agent_runner import (
    autonomous_policy_allowed,
    build_agent_prompt,
    detect_backend,
    run_agent_cycle,
)
from nxl.core.orchestrator import Orchestrator
...
_WORKFLOW_METADATA = ".nxl/skill_pack.json"
_BUILD_PLAN_DIR = "implementation_plan"
_PLAN_FILE = ".nxl/plan.json"
_REFRESH_COOLDOWN_RUNS = 3
_AGENT_LOOP_SLEEP_SECONDS = 2
_RUNTIME_CONTRACTS_DIR = ".nxl/runtime/contracts"
...
def _validate_agent_contract(
    ...
    *,
    ...
    if changed_project_files and not allowed_checks:
        violations.append(
            "project files changed without an allowed `nxl check`"
        )
...
def _trigger_research_and_plan_refresh(project_dir: Path, reason: str, dry_run: bool) -> bool:
    ...
    from nxl.core import plan as plan_mod
    from nxl.core import research as research_mod
...
def run(
    ...
    if not config_dir.is_dir():
        console(
            "Project not initialised. Run `nxl init` first.", "error"
        )
        return 1
