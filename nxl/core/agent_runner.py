...
def build_agent_prompt(
    ...
    return f"""You are the execution engine for NexusLoop.

Work inside this project directory: {project_dir}

This is a single autonomous cycle. Do not call `nxl run` or `nxl resume` recursively.
...
2. Read `.nxl/spec_compact.md` if present, then follow pointed source files when needed.
3. Read `AGENT.md` if present.
4. If `.claude/commands/drl-run.md` exists, follow its project workflow guidance.
5. Read the latest project state from `.nxl/state.json` and recent registry/journal tails.
...
- Do not skip `.nxl/spec_compact.md` when it exists; use it as the compact entry point into the real project spec.
...
- Before any risky action or code/config edit, run `nxl check --project-dir . --action <type> --details '<json>'`.
...
- This cycle is audited automatically. If you bypass `nxl check` or the helper APIs, the controller will mark the cycle failed.
