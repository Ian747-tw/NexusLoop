...
def run(project_dir: Path) -> int:
    ...
    try:
        from nxl.core.run import _sync_state_from_registry
...
def _print_next_hint(state: ProjectState) -> None:
    ...
    if state.current_phase == "research":
        if mode == "build":
            console("Tip: run `nxl run` to let the agent bootstrap research/build from the current spec.", "info")
        else:
            console("Tip: run `nxl run` to start the autonomous keep/discard loop.", "info")
    elif state.current_phase == "baseline":
        console("Tip: run `nxl plan` to review the current plan.", "info")
    elif state.current_phase in ("experimenting", "focused_tuning", "ablation"):
        console("Tip: run `nxl run` to continue experimenting.", "info")
        ...
        console("Run `nxl doctor` to check for issues.", "info")
