...
def _check_config_dir(project_dir: Path) -> CheckResult:
    """Verify .nxl/ directory exists."""
    config_dir = project_dir / ".nxl"
    ok = config_dir.is_dir()
    msg = str(config_dir) if ok else f"{config_dir} missing — run `nxl init`."
    return CheckResult(".nxl/ directory", ok, msg)
...
def _make_yaml_checker(filename: str) -> Callable[[Path], CheckResult]:
    ...
    def _check(project_dir: Path) -> CheckResult:
        path = project_dir / ".nxl" / filename
        if not path.exists():
            return CheckResult(
                filename,
                False,
                f"{path} not found — run `nxl init`.",
            )
...
def _check_state_json(project_dir: Path) -> CheckResult:
    ...
    path = project_dir / ".nxl" / "state.json"
    if not path.exists():
        return CheckResult(
            "state.json",
            False,
            "state.json missing — run `nxl init`.",
        )
    try:
        with path.open("r", encoding="utf-8") as fh:
            data = json.load(fh)
...
def _check_experiment_registry(project_dir: Path) -> CheckResult:
    ...
    if not path.exists():
        return CheckResult(
            ...
            False,
            "logs/experiment_registry.tsv missing — run `nxl init`.",
        )
...
def _check_hard_rules(project_dir: Path) -> CheckResult:
    ...
    if not path.exists():
        return CheckResult(
            ...
            False,
            "NON_NEGOTIABLE_RULES.md missing — run `nxl init`.",
        )
...
def _check_skills_dir(project_dir: Path) -> CheckResult:
    ...
    ok = path.is_dir()
    msg = "skills/ directory present." if ok else "skills/ missing — run `nxl init`."
...
def run(project_dir: Path, fix: bool = False) -> int:
    ...
    if failed == 0:
        ...
        console(
            ...
            "Run `nxl init` to fix missing files.",
            "error",
        )
        return 1
