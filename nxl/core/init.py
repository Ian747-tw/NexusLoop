...
nxl.core.init
--------------------------
Scaffold a new NexusLoop project directory.
...
_RULES_MD = textwrap.dedent("""\
    ...
    <!-- This file is enforced by nxl check and may not be overridden
    ...
    3. **Never disable the permission check** (`nxl check`).
...
def run(
    ...
    refresh:
        Remove NexusLoop-managed config, state, logs, and plugin files
    ...
    console(f"Initialising NexusLoop project at: {project_dir}", "info")

    # Ask for confirmation unless --auto or --skip-onboarding.
    if refresh and not auto and sys.stdin.isatty():
        try:
            answer = input(
                "  Refresh will remove NexusLoop-managed files and re-run onboarding. Continue? [y/N] "
            ).strip().lower()
...
def _refresh_project_managed_files(project_dir: Path) -> None:
    """Remove NexusLoop-managed files without touching user code."""
    from nxl.plugins.installer import remove_managed_plugin_files
...
def _generate_bundled_drl_skills(project_dir: Path, onboarding_result: object) -> None:
    try:
        from nxl.scaffold.generator import ScaffoldGenerator
...
def _install_custom_skill_generator(project_dir: Path, onboarding_result: object) -> None:
    from nxl.plugins.installer import install_skill_generator_backend
...
def _remove_custom_skill_generator(project_dir: Path) -> None:
    path = project_dir / ".nxl" / "backend" / "skill_generator.md"
    if path.exists():
        path.unlink()
...
def _build_scaffold_onboarding_result(project_dir: Path, onboarding_result: object) -> object:
    from nxl.scaffold.generator import OnboardingResult as ScaffoldOnboardingResult
...
def _write_skill_pack_metadata(
    ...
    metadata = {
        ...
        "custom_generator_backend": (
            ".nxl/backend/skill_generator.md"
            if selected_skill_pack == "custom"
            else None
        ),
        "build_plan_folder": (
