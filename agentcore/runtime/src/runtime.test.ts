...
describe("CommandAuthorityService", () => {
  ...
  test("registry classifies critical authority and risk boundaries", () => {
    ...
    expect(service.get("/resume-mark")).toMatchObject({ expected_event_kinds: ["runtime_resume_anchor_marked"] })
    expect(service.get("/scheduler-nav-run")).toMatchObject({ expected_event_kinds: ["runtime_wake_scheduler_navigation_staged_read_started", "runtime_wake_scheduler_navigation_staged_read_succeeded", "runtime_wake_scheduler_navigation_staged_read_failed"] })
    expect(service.get("/scheduler-nav-write-run")).toMatchObject({ expected_event_kinds: ["runtime_wake_scheduler_navigation_write_run_started", "runtime_wake_scheduler_navigation_write_run_succeeded", "runtime_wake_scheduler_navigation_write_run_failed", "runtime_wake_scheduler_navigation_write_run_blocked"] })
    expect(service.get("/scheduler-nav-checkpoint-run")).toMatchObject({ expected_event_kinds: ["runtime_wake_scheduler_navigation_checkpoint_write_run_started", "runtime_wake_scheduler_navigation_checkpoint_write_run_succeeded", "runtime_wake_scheduler_navigation_checkpoint_write_run_failed", "runtime_wake_scheduler_navigation_checkpoint_write_run_blocked", "runtime_checkpoint_created"] })
...
describe("RuntimeServer core", () => {
  ...
  test("wake assessment previews from resume anchor and creates durable read-only assessment", async () => {
    const dir = await tempProject()
    await makeProject(dir, { approvedSpec: true })
    const adapter = new LongLivedAdapter()
    const server = new RuntimeServer({
      ...
      adapter,
      researchProjectionMode: "disabled",
      runtimeCheckpointId: () => "checkpoint_wake_1",
      runtimeResumeId: () => "resume_wake_1",
      runtimeWakeId: () => "wake_1",
      runtimeCheckpointNow: () => new Date("2026-05-10T12:00:00.000Z"),
      runtimeResumeNow: () => new Date("2026-05-10T12:05:00.000Z"),
      runtimeWakeNow: () => new Date("2026-05-10T12:10:00.000Z"),
    })
    await server.start()
    await server.command("runtime.create_runtime_checkpoint", { scope: "full", reason: "wake token=checkpoint-secret", requestedBy: "operator" })
    await server.command("runtime.mark_checkpoint_resume_anchor", { checkpointId: "checkpoint_wake_1", requestedBy: "operator" })
    const eventsBeforePreview = await readJsonlEvents(dir)
    const preview = await server.command("runtime.preview_wake_assessment", { resumeId: "resume_wake_1", requestedBy: "operator" }) as {
      allowed: boolean
      resume_id: string
    ...
    expect(preview).toMatchObject({ allowed: true, resume_id: "resume_wake_1", checkpoint_id: "checkpoint_wake_1", drift_status: "advanced" })
    ...
    const assessment = await server.command("runtime.create_wake_assessment", { resumeId: "resume_wake_1", requestedBy: "operator" }) as {
      wake_id: string
      allowed: boolean
      assessment_hash: string
      sections: { resume?: { resume_id?: string }; reasoning?: { health_status?: string } }
    }
    expect(assessment.wake_id).toBe("wake_1")
    expect(assessment.allowed).toBe(true)
    expect(assessment.assessment_hash).toMatch(/^[a-f0-9]{64}$/)
    expect(assessment.sections.resume?.resume_id).toBe("resume_wake_1")
    expect(assessment.sections.reasoning?.health_status).toBe("ok")
    expect(await server.command("runtime.list_recent_missions", { limit: 20 })).toEqual(beforeMissions)
    expect(adapter.packets).toHaveLength(beforePackets)
    const events = await readJsonlEvents(dir)
    expect(events.at(-1)).toMatchObject({ kind: "runtime_wake_assessment_created", wake_id: "wake_1", resume_id: "resume_wake_1", checkpoint_id: "checkpoint_wake_1" })
    expect(JSON.stringify(events)).not.toContain("checkpoint-secret")
    const listed = await server.command("runtime.list_wake_assessments", { limit: 10 }) as Array<{ wake_id: string }>
    expect(listed.map((item) => item.wake_id)).toEqual(["wake_1"])
    const selected = await server.command("runtime.get_wake_assessment", { wakeId: "wake_1" })
    expect(selected).toMatchObject({ wake_id: "wake_1", resume_id: "resume_wake_1" })
    await server.shutdown()
  })

  test("wake assessment checkpoint-only preview is read-only but create requires resume anchor", async () => {
    const dir = await tempProject()
    await makeProject(dir, { approvedSpec: true })
    const server = new RuntimeServer({
      projectDir: dir,
      mode: "active",
      researchProjectionMode: "disabled",
      runtimeCheckpointId: () => "checkpoint_wake_preview_1",
    })
    await server.start()
    await server.command("runtime.create_runtime_checkpoint", { scope: "full", requestedBy: "operator" })
    const eventsBeforePreview = await readJsonlEvents(dir)
    const preview = await server.command("runtime.preview_wake_assessment", { checkpointId: "checkpoint_wake_preview_1", requestedBy: "operator" }) as {
      allowed: boolean
      warnings: string[]
    }
    expect(preview.allowed).toBe(true)
    expect(preview.warnings).toContain("wake preview is using an unanchored checkpoint; create requires resume_id")
    expect(await readJsonlEvents(dir)).toHaveLength(eventsBeforePreview.length)
    await expect(server.command("runtime.create_wake_assessment", { checkpointId: "checkpoint_wake_preview_1", requestedBy: "operator" })).rejects.toThrow("resume_id is required for wake assessment creation")
    await server.shutdown()
  })

  test("wake assessment blocks tampered checkpoints and enforces write gates", async () => {
    const dir = await tempProject()
    await makeProject(dir, { approvedSpec: true })
    const server = new RuntimeServer({
      projectDir: dir,
      mode: "active",
      researchProjectionMode: "disabled",
      runtimeCheckpointId: () => "checkpoint_wake_tamper_1",
      runtimeResumeId: () => "resume_wake_tamper_1",
    })
    await server.start()
    await server.command("runtime.create_runtime_checkpoint", { scope: "full", reason: "token=original-wake-secret", requestedBy: "operator" })
    await server.command("runtime.mark_checkpoint_resume_anchor", { checkpointId: "checkpoint_wake_tamper_1", requestedBy: "operator" })
    const eventsPath = join(dir, ".nxl", "events.jsonl")
    const rewritten = (await readFile(eventsPath, "utf8")).split(/\r?\n/).filter(Boolean).map((line) => {
      const event = JSON.parse(line) as Record<string, unknown>
      if (event.kind === "runtime_checkpoint_created" && event.checkpoint && typeof event.checkpoint === "object") {
        ;(event.checkpoint as Record<string, unknown>).reason = "token=tampered-wake-secret"
      }
      return JSON.stringify(event)
    }).join("\n") + "\n"
    await writeFile(eventsPath, rewritten)
    const preview = await server.command("runtime.preview_wake_assessment", { resumeId: "resume_wake_tamper_1", requestedBy: "operator" }) as { allowed: boolean; blockers: string[] }
    expect(preview.allowed).toBe(false)
    expect(preview.blockers).toContain("runtime checkpoint hash verification failed")
    expect(JSON.stringify(preview)).not.toContain("tampered-wake-secret")
    await expect(server.command("runtime.create_wake_assessment", { resumeId: "resume_wake_tamper_1", requestedBy: "operator" })).rejects.toThrow("runtime checkpoint hash verification failed")
    await server.shutdown()

    const viewServer = new RuntimeServer({ projectDir: dir, mode: "view-records", researchProjectionMode: "disabled" })
    const viewPreview = await viewServer.command("runtime.preview_wake_assessment", { resumeId: "resume_wake_tamper_1", requestedBy: "operator" }) as { allowed: boolean }
  ...
  test("wake scheduler audit links recovery workflows and reports advisory gaps", async () => {
    ...
    await makeProject(dir, { approvedSpec: true })
    const eventStore = new EventStore(join(dir, ".nxl", "events.jsonl"))
    await eventStore.append({ kind: "runtime_wake_scheduler_stale_run_detected", created_at: "2026-05-11T15:00:00.000Z", stale_prior_run: { detected: true, prior_event_id: "prior_event_audit_1", prior_tick_id: "prior_tick_audit_1" } })
    await eventStore.append({ kind: "runtime_wake_scheduler_recovery_recorded", created_at: "2026-05-11T15:01:00.000Z", recovery_id: "recovery_audit_1", recovery_hash: "hash_audit_1", resolution: "resolved", stale_prior_event_id: "prior_event_audit_1", prior_tick_id: "prior_tick_audit_1" })
  ...
  test("wake scheduler audit summary uses lifecycle event scheduler statuses", async () => {
    const heartbeatDir = await tempProject()
    await makeProject(heartbeatDir, { approvedSpec: true })
    const heartbeatStore = new EventStore(join(heartbeatDir, ".nxl", "events.jsonl"))
    await heartbeatStore.append({ kind: "runtime_wake_scheduler_started", created_at: "2026-05-11T15:00:00.000Z", scheduler_status: "running" })
    await heartbeatStore.append({ kind: "runtime_wake_scheduler_heartbeat", created_at: "2026-05-11T15:01:00.000Z", scheduler_status: "running" })
    const heartbeatServer = new RuntimeServer({ projectDir: heartbeatDir, mode: "status", researchProjectionMode: "disabled" })
    await heartbeatServer.start()
    await expect(heartbeatServer.command("runtime.wake_scheduler_audit_summary")).resolves.toMatchObject({ latest_scheduler_status: "running" })
    await heartbeatServer.shutdown()

    const failedStopDir = await tempProject()
    await makeProject(failedStopDir, { approvedSpec: true })
    const failedStopStore = new EventStore(join(failedStopDir, ".nxl", "events.jsonl"))
    await failedStopStore.append({ kind: "runtime_wake_scheduler_started", created_at: "2026-05-11T15:00:00.000Z", scheduler_status: "running" })
    await failedStopStore.append({ kind: "runtime_wake_scheduler_stopped", created_at: "2026-05-11T15:01:00.000Z", scheduler_status: "failed" })
    const failedStopServer = new RuntimeServer({ projectDir: failedStopDir, mode: "status", researchProjectionMode: "disabled" })
    await failedStopServer.start()
    await expect(failedStopServer.command("runtime.wake_scheduler_audit_summary")).resolves.toMatchObject({ latest_scheduler_status: "failed" })
    await failedStopServer.shutdown()
  })
  ...
  test("wake scheduler audit validates filters clearly", async () => {
    ...
    await makeProject(dir, { approvedSpec: true })
    const eventStore = new EventStore(join(dir, ".nxl", "events.jsonl"))
    await eventStore.append({ kind: "runtime_wake_schedule_created", created_at: "2026-05-11T15:00:00.000Z", schedule_id: "schedule_related_alias" })
    await eventStore.append({ kind: "runtime_wake_schedule_created", created_at: "2026-05-11T15:01:00.000Z", schedule_id: "schedule_unrelated_alias" })
    const server = new RuntimeServer({ projectDir: dir, mode: "status", researchProjectionMode: "disabled" })
    await server.start()
    const relatedAlias = await server.command("runtime.wake_scheduler_audit_timeline", { related: "schedule_related_alias" }) as Array<{ related_ids: Record<string, string[]> }>
    expect(relatedAlias).toHaveLength(1)
    expect(relatedAlias[0]?.related_ids.schedule_id).toEqual(["schedule_related_alias"])
  ...
  test("wake scheduler navigation staged read comparison groups history and computes stable content hashes", async () => {
    const dir = await tempProject()
    await makeProject(dir, { approvedSpec: true })
    const eventStore = new EventStore(join(dir, ".nxl", "events.jsonl"))
    await eventStore.append({
      kind: "runtime_wake_scheduler_navigation_command_staged",
      staged_id: "staged_status_compare",
      command: "/scheduler-status",
      command_type: "read",
      risk: "safe_read",
      target_kind: "scheduler_status",
      label: "status",
      notes: [],
      staged_at: "2026-05-12T12:00:00.000Z",
      staged_by: "fixture",
      stage_hash: createHash("sha256").update("/scheduler-status").digest("hex"),
    })
    for (const [index, summary] of ["status=stopped", "status=stopped", "status=running"].entries()) {
      await eventStore.append({
        kind: "runtime_wake_scheduler_navigation_staged_read_succeeded",
        run_id: `compare_run_${index + 1}`,
        staged_id: "staged_status_compare",
        command: "/scheduler-status",
        target_kind: "scheduler_status",
        status: "succeeded",
        ...
        started_at: `2026-05-12T12:0${index}:00.000Z`,
        completed_at: `2026-05-12T12:0${index}:01.000Z`,
    ...
    await eventStore.append({
      ...
      run_id: "compare_failed_secret",
      staged_id: "staged_status_compare",
      command: "/scheduler-status",
      target_kind: "scheduler_status",
      status: "failed",
      error: "token=abc123 status failed",
      started_at: "2026-05-12T12:03:00.000Z",
      completed_at: "2026-05-12T12:03:01.000Z",
    ...
    const server = new RuntimeServer({
      ...
      mode: "status",
      researchProjectionMode: "disabled",
      runtimeWakeSchedulerNow: () => new Date("2026-05-12T14:00:00.000Z"),
    })
    await server.start()
    ...
    const unchanged = await server.command("runtime.wake_scheduler_navigation_staged_read_compare", { left_run_id: "compare_run_1", right_run_id: "compare_run_2" }) as { comparison_status: string; left_comparison_hash: string; right_comparison_hash: string }
    expect(unchanged.comparison_status).toBe("unchanged")
    expect(unchanged.left_comparison_hash).toBe(unchanged.right_comparison_hash)
    const changed = await server.command("runtime.wake_scheduler_navigation_staged_read_compare", { left_run_id: "compare_run_2", right_run_id: "compare_run_3" }) as { comparison_status: string; left_comparison_hash: string; right_comparison_hash: string }
    expect(changed.comparison_status).toBe("changed")
    expect(changed.left_comparison_hash).not.toBe(changed.right_comparison_hash)
    const failedChanged = await server.command("runtime.wake_scheduler_navigation_staged_read_compare", { left_run_id: "compare_run_3", right_run_id: "compare_failed_secret" }) as { comparison_status: string; summary_delta: string }
    ...
    await server.command("runtime.wake_scheduler_navigation_staged_read_stale", { stale_after_ms: 3_600_000 })
    await server.command("runtime.wake_scheduler_navigation_staged_read_group", { staged_id: "staged_status_compare" })
    const eventsAfter = await readJsonlEvents(dir)
    expect(eventsAfter.length).toBe(eventsBefore.length)
    expect(eventsAfter.map((event) => event.kind)).not.toContain("runtime_wake_scheduler_navigation_staged_read_started")
  ...
  test("wake scheduler navigation staged read comparison handles first run stale no-run commands and read-mode access", async () => {
    const dir = await tempProject()
    await makeProject(dir, { approvedSpec: true })
    const server = new RuntimeServer({
      projectDir: dir,
      mode: "active",
      researchProjectionMode: "disabled",
      runtimeWakeSchedulerNow: () => new Date("2026-05-12T15:00:00.000Z"),
    })
    await server.start()
    const staged = await server.command("runtime.stage_wake_scheduler_navigation_command", { command: "/scheduler-status", requested_by: "operator-compare" }) as { staged_id: string }
    const noRunStale = await server.command("runtime.wake_scheduler_navigation_staged_read_stale", { stale_after_ms: 3_600_000 }) as Array<{ staged_id: string; stale: boolean; latest_run_id?: string; recommended_commands: Array<{ command: string; command_type: string; requires_active_runtime?: boolean }> }>
    expect(noRunStale).toHaveLength(1)
    expect(noRunStale[0]).toMatchObject({ staged_id: staged.staged_id, stale: true })
    expect(noRunStale[0].latest_run_id).toBeUndefined()
    expect(noRunStale[0].recommended_commands.some((command) => command.command === `/scheduler-nav-run ${staged.staged_id}` && command.command_type === "write" && command.requires_active_runtime)).toBe(true)

    const run = await server.command("runtime.execute_wake_scheduler_navigation_staged_read", { staged_id: staged.staged_id, requested_by: "operator-compare" }) as { run_id: string }
    const first = await server.command("runtime.wake_scheduler_navigation_staged_read_compare", { staged_id: staged.staged_id }) as { comparison_status: string; left_run_id: string; right_run_id: string }
    expect(first).toMatchObject({ comparison_status: "first_run", left_run_id: run.run_id, right_run_id: run.run_id })
    const group = await server.command("runtime.wake_scheduler_navigation_staged_read_group", { staged_id: staged.staged_id }) as { staged_id: string; run_count: number; comparison_status: string }
    expect(group).toMatchObject({ staged_id: staged.staged_id, run_count: 1, comparison_status: "first_run" })
    await expect(server.command("runtime.wake_scheduler_navigation_staged_read_compare", { staged_id: "missing" })).rejects.toThrow("no terminal runs")
    await server.shutdown()

    const statusServer = new RuntimeServer({ projectDir: dir, mode: "status", researchProjectionMode: "disabled" })
    await statusServer.start()
    await expect(statusServer.command("runtime.wake_scheduler_navigation_staged_read_history", {})).resolves.toMatchObject({ total_groups: 1 })
    await expect(statusServer.command("runtime.wake_scheduler_navigation_staged_read_group", { staged_id: staged.staged_id })).resolves.toMatchObject({ staged_id: staged.staged_id })
    await statusServer.shutdown()
  })
  ...
  test("wake scheduler navigation write staging persists allowed intent only and blocks execution surfaces", async () => {
    const dir = await tempProject()
    await makeProject(dir, { approvedSpec: true })

    const statusServer = new RuntimeServer({ projectDir: dir, mode: "status", researchProjectionMode: "disabled" })
    await statusServer.start()
    const statusPreview = await statusServer.command("runtime.preview_wake_scheduler_navigation_write_stage", { command: "/wake-tick-dry-run" }) as { eligibility: { can_stage: boolean; risk: string } }
    expect(statusPreview.eligibility).toMatchObject({ can_stage: true, risk: "low_risk_write" })
    const statusList = await statusServer.command("runtime.list_wake_scheduler_navigation_staged_write_commands", { limit: 10 }) as unknown[]
    expect(statusList).toEqual([])
    await expect(statusServer.command("runtime.stage_wake_scheduler_navigation_write_command", { command: "/wake-tick-dry-run" })).rejects.toThrow(/requires active mode/)
    await statusServer.shutdown()

    const server = new RuntimeServer({ projectDir: dir, mode: "active", researchProjectionMode: "disabled" })
    await server.start()

    const lowPreview = await server.command("runtime.preview_wake_scheduler_navigation_write_stage", { command: "/wake-tick-dry-run" }) as { eligibility: { can_stage: boolean; authority_gate: string; blockers: string[] } }
    expect(lowPreview.eligibility).toMatchObject({ can_stage: true, authority_gate: "wake_schedule_tick" })
    const dryRun = await server.command("runtime.stage_wake_scheduler_navigation_write_command", { command: "/wake-tick-dry-run", requestedBy: "operator" }) as { staged_write_id: string; command: string; risk: string; authority_gate: string; summary_preview: string }
    expect(dryRun).toMatchObject({ command: "/wake-tick-dry-run", risk: "low_risk_write", authority_gate: "wake_schedule_tick" })
    const dryRunAgain = await server.command("runtime.stage_wake_scheduler_navigation_write_command", { command: "/wake-tick-dry-run", requestedBy: "operator" }) as { staged_write_id: string }
    expect(dryRunAgain.staged_write_id).toBe(dryRun.staged_write_id)

    const stagedReadRun = await server.command("runtime.stage_wake_scheduler_navigation_write_command", { command: "/scheduler-nav-run staged_read_7u_1", requestedBy: "operator" }) as { command: string; risk: string }
    ...
    for (const command of ["/scheduler-recovery-ack recovery_7u_1", "/scheduler-recovery-step-done workflow_7u_1 0", "/continue-plan wake=wake_7u_1"]) {
      ...
      expect(staged.risk).toBe("medium_risk_write")
    }

    for (const command of ["/scheduler-start every=60s", "/scheduler-stop", "/wake-tick", "/continue-step plan_7u_1", "/handoff token=abc123", "/proposal-review proposal_1 token=abc123", "/apply-proposal proposal_1", "/apply-target proposal proposal_1", "/complete mission_1", "/fail mission_1", "/cancel mission_1", "/synthesize token=abc123", "/cycle", "/api-call token=abc123", "/tmp/repro", "/path/foo", "scheduler-start"]) {
      await expect(server.command("runtime.stage_wake_scheduler_navigation_write_command", { command, allowMediumRisk: true, requestedBy: "operator" })).rejects.toThrow(/cannot be staged/)
    }

    const list = await server.command("runtime.list_wake_scheduler_navigation_staged_write_commands", { limit: 20 }) as Array<{ staged_write_id: string; command: string }>
    expect(list.length).toBe(6)
    expect(list.some((item) => item.command === "/wake-tick-dry-run")).toBe(true)
    const selected = await server.command("runtime.get_wake_scheduler_navigation_staged_write_command", { stagedWriteId: dryRun.staged_write_id }) as { staged_write_id: string; command: string }
    expect(selected).toMatchObject({ staged_write_id: dryRun.staged_write_id, command: "/wake-tick-dry-run" })

    const removed = await server.command("runtime.remove_wake_scheduler_navigation_staged_write_command", { stagedWriteId: dryRun.staged_write_id, reason: "token=abc123", requestedBy: "operator" }) as { staged_write_id: string } | null
    expect(removed?.staged_write_id).toBe(dryRun.staged_write_id)
    const afterRemove = await server.command("runtime.list_wake_scheduler_navigation_staged_write_commands", { limit: 20 }) as Array<{ staged_write_id: string }>
    expect(afterRemove.some((item) => item.staged_write_id === dryRun.staged_write_id)).toBe(false)
    const afterClear = await server.command("runtime.clear_wake_scheduler_navigation_staged_write_commands", { reason: "token=abc123", requestedBy: "operator" }) as unknown[]
    expect(afterClear).toEqual([])
    const finalList = await server.command("runtime.list_wake_scheduler_navigation_staged_write_commands", { limit: 20 }) as unknown[]
    expect(finalList).toEqual([])

    const events = await readJsonlEvents(dir)
    const eventKinds = events.map((event) => event.kind)
    expect(eventKinds).toContain("runtime_wake_scheduler_navigation_write_command_staged")
    expect(eventKinds).toContain("runtime_wake_scheduler_navigation_write_command_removed")
    expect(eventKinds).toContain("runtime_wake_scheduler_navigation_write_commands_cleared")
    for (const forbidden of ["runtime_wake_scheduler_started", "runtime_wake_schedule_tick_completed", "runtime_checkpoint_created", "runtime_wake_scheduler_recovery_recorded", "runtime_wake_scheduler_recovery_workflow_created", "runtime_wake_scheduler_recovery_workflow_step_recorded", "runtime_continuation_plan_created", "runtime_continuation_step_started", "runtime_opencode_handoff_sent", "runtime_mission_completed", "runtime_mission_failed"]) {
  ...
  test("wake scheduler navigation checkpoint write comparison audits outcomes artifacts and approvals read-only", async () => {
    ...
    await eventStore.append({
      ...
      authority_gate: "checkpoint_runtime",
      target_kind: "checkpoint",
      staged_at: "2026-05-16T07:00:00.000Z",
      staged_by: "fixture",
      status: "staged",
      stage_hash: "stage_hash_removed",
      summary_preview: "/checkpoint full removed",
    })
    await eventStore.append({
      kind: "runtime_wake_scheduler_navigation_write_approval_recorded",
      approval_id: "approval_checkpoint_removed",
      staged_write_id: "staged_checkpoint_removed",
      command: "/checkpoint full removed",
      command_name: "/checkpoint",
      risk: "medium_risk_write",
      ...
      stage_hash: "stage_hash_removed",
      status: "approved",
      approved_at: "2026-05-16T07:05:00.000Z",
      requested_by: "fixture",
      evidence: [],
      approval_hash: "approval_hash_removed",
      expires_at: "2026-05-17T07:05:00.000Z",
      created_at: "2026-05-16T07:05:00.000Z",
    })
    await eventStore.append({
    ...
    expect(usage.approvals.find((approval) => approval.approval_id === "approval_checkpoint_removed")).toMatchObject({ approval_status: "expired", expired_before_use: true })
    expect(usage.approvals.find((approval) => approval.approval_id === "approval_checkpoint_removed")?.warnings.join(" ")).toContain("no longer active")
...
describe("RuntimeServerClient", () => {
  ...
  test("executor review proposal review request creates one review request for an 8H proposal", async () => {
    ...
    const adapter = new LongLivedAdapter()
    const server = new RuntimeServer({ projectDir: dir, adapter, researchProjectionMode: "disabled" })
    const seedReview = async (suffix: string) => {
      await server.eventStore.append({
        kind: "work_intent_created",
        intent: { intent_id: `intent_decision_${suffix}`, kind: "user_message", message: `executor review proposal review decision fixture ${suffix}`, created_at: "2026-06-26T00:00:00.000Z", status: "created" },
      } as JsonlEvent)
      await server.eventStore.append({
        kind: "mission_created",
        mission: { mission_id: `mission_decision_${suffix}`, intent_id: `intent_decision_${suffix}`, project_dir: dir, objective: `novel generic executor review proposal ${suffix}`, status: "sent", created_at: "2026-06-26T00:00:00.000Z", updated_at: "2026-06-26T00:00:00.000Z", sent_at: "2026-06-26T00:00:00.000Z" },
      } as JsonlEvent)
      await server.eventStore.append({
        kind: "mission_claimed",
        claim: { claim_id: `claim_decision_${suffix}`, mission_id: `mission_decision_${suffix}`, executor_id: "executor", claimed_at: "2026-06-26T00:00:00.000Z", status: "active" },
      } as JsonlEvent)
      await server.eventStore.append({
        kind: "mission_result_submitted",
        result: { result_id: `result_decision_${suffix}`, mission_id: `mission_decision_${suffix}`, claim_id: `claim_decision_${suffix}`, summary: `novel generic executor result ${suffix}`, created_at: "2026-06-26T00:00:00.000Z", status: "submitted" },
      } as JsonlEvent)
      await server.eventStore.append({
        kind: "commander_executor_review_succeeded",
        review_id: `executor_review_decision_${suffix}`,
        packet_id: `packet_decision_${suffix}`,
        packet_status: "ready_for_commander_review",
        status: "succeeded",
        provider_kind: "fake-provider",
        ...
        evidence_ids: [`mission_result:result_decision_${suffix}`],
        recommended_commands: [],
        started_at: "2026-06-26T00:00:00.000Z",
        completed_at: "2026-06-26T00:00:01.000Z",
        requested_by: "operator",
      ...
      const draftPreview = await server.command("runtime.preview_executor_review_proposal_drafts", { review_id: `executor_review_decision_${suffix}` }) as { candidates: Array<{ draft_id: string }> }
      const draftId = draftPreview.candidates[0]?.draft_id
      expect(typeof draftId).toBe("string")
      return { suffix, draftId: draftId! }
    }
    const createRequest = async (seed: { suffix: string; draftId: string }) => {
      const created = await server.command("runtime.create_executor_review_proposal", { review_id: `executor_review_decision_${seed.suffix}`, draft_id: seed.draftId, requested_by: "alice" }) as { status: string; proposal_id: string; create_id: string }
      expect(created.status).toBe("created")
      const requested = await server.command("runtime.request_executor_review_proposal_review", { proposal_id: created.proposal_id, requested_by: "bob" }) as { status: string; review_request_id: string; request_gate_id: string }
      expect(requested.status).toBe("requested")
    ...
    await expect(server.command("runtime.preview_executor_review_proposal_review_decision", {
      ...
      can_decide: true,
      proposal_id: approveFixture.created.proposal_id,
      request_gate_id: approveFixture.requested.request_gate_id,
      source_executor_review_id: "executor_review_decision_approve",
      source_draft_id: approveFixture.draftId,
    })
    await expect(server.command("runtime.preview_executor_review_proposal_review_decision", {
      review_request_id: approveFixture.requested.review_request_id,
      decision: "bogus",
    ...
    await expect(server.command("runtime.decide_executor_review_proposal_review", {
      ...
      decision: "approve",
      dry_run: true,
      decided_by: "dry-runner",
    })).resolves.toMatchObject({
      status: "dry_run",
      decision: "approve",
      review_request_id: approveFixture.requested.review_request_id,
    })
    expect(await readEventKinds(dir)).toEqual(beforeDecisionKinds)

    const approved = await server.command("runtime.decide_executor_review_proposal_review", {
      review_request_id: approveFixture.requested.review_request_id,
      decision: "approve",
      decided_by: "alice token=decision-secret",
    }) as { status: string; decision_gate_id: string; proposal_id?: string; decision_hash?: string }
    ...
    await expect(server.command("runtime.get_executor_review_proposal_review_decision", { decision_gate_id: approved.decision_gate_id })).resolves.toMatchObject({
      status: "approved",
      review_request_id: approveFixture.requested.review_request_id,
    })
    await expect(server.command("runtime.preview_executor_review_proposal_review_decision", {
      ...
      blockers: expect.arrayContaining(["review request already approved"]),
    })
    ...
    await expect(server.command("runtime.preview_executor_review_proposal_apply_readiness", {
	  ...
	  test("executor review proposal narrow apply marks approved generic proposal applied without mission mutation", async () => {
    const dir = await tempProject()
    await makeProject(dir, { approvedSpec: true })
    const adapter = new LongLivedAdapter()
    const server = new RuntimeServer({ projectDir: dir, adapter, researchProjectionMode: "disabled" })
    await server.eventStore.append({
      kind: "commander_executor_review_succeeded",
      review_id: "executor_review_narrow_apply_1",
      packet_id: "packet_narrow_apply_1",
      packet_status: "ready_for_commander_review",
      status: "succeeded",
      provider_kind: "fake-provider",
      decision: "needs_human_review",
      confidence: 0.86,
      summary: "Human review accepted for a generic executor outcome token=narrow-secret",
      findings: [{ finding_id: "finding_narrow_apply_1", severity: "warning", title: "Inspect", summary: "Operator should inspect the bounded result.", evidence_ids: ["manual_note:narrow_apply_1"], recommended_commands: [] }],
      evidence_ids: ["manual_note:narrow_apply_1"],
      recommended_commands: [],
      started_at: "2026-06-26T00:00:00.000Z",
      completed_at: "2026-06-26T00:00:01.000Z",
      requested_by: "operator",
      review_hash: "review_hash_narrow_apply_1",
    } as JsonlEvent)
    const draftPreview = await server.command("runtime.preview_executor_review_proposal_drafts", { review_id: "executor_review_narrow_apply_1" }) as { candidates: Array<{ draft_id: string; draft_kind: string }> }
    const draftId = draftPreview.candidates[0]?.draft_id
    expect(typeof draftId).toBe("string")
    expect(draftPreview.candidates[0]?.draft_kind).toBe("human_review")

    await server.start()
    const created = await server.command("runtime.create_executor_review_proposal", { review_id: "executor_review_narrow_apply_1", draft_id: draftId, requested_by: "alice" }) as { status: string; proposal_id: string }
    expect(created.status).toBe("created")
    ...
    const readiness = await server.command("runtime.preview_executor_review_proposal_apply_readiness", { proposal_id: created.proposal_id }) as { readiness_id: string; status: string; candidate_kind: string }
    expect(readiness).toMatchObject({ status: "ready", candidate_kind: "human_review" })
    await expect(server.command("runtime.preview_executor_review_proposal_narrow_apply", { proposal_id: created.proposal_id, readiness_id: readiness.readiness_id })).resolves.toMatchObject({
      status: "ready",
      can_apply: true,
    ...
    const beforeApplyKinds = await readEventKinds(dir)
    await expect(server.command("runtime.apply_executor_review_proposal_narrow", { proposal_id: created.proposal_id, dry_run: true, applied_by: "dry-runner token=narrow-secret" })).resolves.toMatchObject({
      status: "dry_run",
      proposal_id: created.proposal_id,
    })
    expect(await readEventKinds(dir)).toEqual(beforeApplyKinds)
    const applied = await server.command("runtime.apply_executor_review_proposal_narrow", { proposal_id: created.proposal_id, applied_by: "operator token=narrow-secret" }) as { status: string; apply_id: string; proposal_id: string }
    expect(applied).toMatchObject({ status: "applied", proposal_id: created.proposal_id })
    await expect(server.command("runtime.get_commander_proposal", { proposal_id: created.proposal_id })).resolves.toMatchObject({
      status: "applied",
      application_result: `executor_review_narrow_apply:${applied.apply_id}`,
    })
    await expect(server.command("runtime.list_executor_review_proposal_narrow_applies", { proposal_id: created.proposal_id })).resolves.toEqual([
      expect.objectContaining({ status: "applied", proposal_id: created.proposal_id, apply_id: applied.apply_id }),
    ])
    await expect(server.command("runtime.get_executor_review_proposal_narrow_apply", { apply_id: applied.apply_id })).resolves.toMatchObject({
      status: "applied",
      proposal_id: created.proposal_id,
    })
    ...
    const addedKinds = kinds.slice(beforeApplyKinds.length)
    expect(addedKinds).not.toContain("mission_progress_recorded")
    expect(addedKinds).not.toContain("mission_result_submitted")
    expect(addedKinds).not.toContain("runtime_checkpoint_created")
    expect(addedKinds).not.toContain("external_api_request_executed")
    expect(addedKinds).not.toContain("opencode_handoff_started")
    expect(JSON.stringify(await server.eventStore.readAll())).not.toContain("narrow-secret")
    await server.shutdown()
  })
  ...
  test("executor review proposal create runtime client commands do not auto-start OpenCode", async () => {
    ...
    await expect(client.command("runtime.preview_executor_review_proposal_review_decision", { review_request_id: "missing-review", decision: "approve" })).resolves.toMatchObject({
      status: "blocked",
      can_decide: false,
    })
    await expect(client.command("runtime.decide_executor_review_proposal_review", { review_request_id: "missing-review", decision: "approve", dry_run: true })).resolves.toMatchObject({
      status: "blocked",
      error: expect.stringContaining("review_request_id was not found"),
    })
    ...
    await expect(client.command("runtime.preview_executor_review_proposal_narrow_apply", { proposal_id: "missing-proposal" })).resolves.toMatchObject({
      status: "blocked",
      can_apply: false,
    })
    await expect(client.command("runtime.apply_executor_review_proposal_narrow", { proposal_id: "missing-proposal", dry_run: true })).resolves.toMatchObject({
      status: "blocked",
      error: expect.stringContaining("apply readiness is blocked"),
    })
    await expect(client.command("runtime.list_executor_review_proposal_narrow_applies")).resolves.toEqual([])
    await expect(client.command("runtime.get_executor_review_proposal_narrow_apply", { apply_id: "missing-apply" })).resolves.toBeNull()
    await expect(client.command("runtime.apply_executor_review_proposal_narrow", { proposal_id: "missing-proposal", applied_by: "operator" })).rejects.toThrow("runtime must be started before proposal writes")
    expect(adapter.startCalls).toBe(0)
    expect(await readEventKinds(dir)).not.toContain("runtime_started")
    await client.shutdown()
  })
  ...
  test("authority registry includes executor review proposal review decision commands", () => {
    const authority = new CommandAuthorityService()
    expect(authority.get("/executor-review-proposal-review-approve")).toMatchObject({
      risk: "high_impact_write",
      mutates_events: true,
      runtime_command: "runtime.decide_executor_review_proposal_review",
      gate: "proposal_review_runtime",
    })
    expect(authority.get("/executor-review-proposal-review-reject")).toMatchObject({
      risk: "high_impact_write",
      mutates_events: true,
      runtime_command: "runtime.decide_executor_review_proposal_review",
      gate: "proposal_review_runtime",
    })
    expect(authority.get("/executor-review-proposal-review-decision-dry-run")).toMatchObject({
      risk: "safe_read",
      mutates_events: false,
    })
  ...
  test("authority registry includes executor review proposal narrow apply commands", () => {
    const authority = new CommandAuthorityService()
    expect(authority.get("/executor-review-proposal-narrow-apply-preview")).toMatchObject({
      risk: "safe_read",
      mutates_events: false,
      runtime_command: "runtime.preview_executor_review_proposal_narrow_apply",
      gate: "none",
    })
    expect(authority.get("/executor-review-proposal-narrow-apply")).toMatchObject({
      risk: "high_impact_write",
      mutates_events: true,
      runtime_command: "runtime.apply_executor_review_proposal_narrow",
      gate: "proposal_review_runtime",
    })
    expect(authority.get("/executor-review-proposal-narrow-apply-dry-run")).toMatchObject({
      risk: "safe_read",
      mutates_events: false,
      runtime_command: "runtime.apply_executor_review_proposal_narrow",
    })
    expect(authority.validationProfile("/executor-review-proposal-narrow-apply").targeted_e2e).toContain("tests/e2e_user/scenarios/test_executor_review_proposal_narrow_apply_tui.py")
  })
