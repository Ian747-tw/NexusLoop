...
export class ExternalApiRequestService {
  ...
  private build(input: ExternalApiRequestInput): BuiltRequest {
    ...
    for (const [key, value] of Object.entries(input.headers ?? {})) {
      ...
      else if (DANGEROUS_USER_HEADERS.has(key.toLowerCase()) && !credentialHeaderTargets.has(key.toLowerCase())) blockers.push(`header is not allowed from user input: ${key}`)
