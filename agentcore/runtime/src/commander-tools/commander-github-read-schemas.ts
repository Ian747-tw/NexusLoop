...
const file = object("Bounded changed-file summary", {
  filename: string("Repository-relative filename", 240), status: string("Change status", 64), additions: integer("Added lines"), deletions: integer("Deleted lines"), changes: integer("Changed lines"),
...
const operationEvidence: Record<CommanderGithubReadToolId, CommanderToolJsonSchemaProperty> = {
  "github.repository_get": object("Repository metadata evidence", { full_name: string("Canonical repository", 201), name: string("Repository name", 120), description_preview: string("Bounded repository description", 500), default_branch: string("Default branch", 120), visibility: string("Visibility", 32), archived: boolean("Archived state"), private: boolean("Private state"), truncated: boolean("Truncation state") }, ["full_name", "name", "default_branch", "visibility", "archived", "private", "truncated"]),
  "github.commit_get": object("Exact commit metadata evidence", { sha: string("Exact commit SHA", 40), observed_commit_sha: string("Observed exact commit SHA", 40), message_preview: string("Bounded commit message", 500), author_name_preview: string("Bounded commit author name", 120), authored_at: string("Authored timestamp", 64), parent_shas: array("Parent SHAs", string("Exact parent SHA", 40)), omitted_parent_count: integer("Omitted parent count", 100), truncated: boolean("Truncation state") }, ["sha", "observed_commit_sha", "message_preview", "author_name_preview", "authored_at", "parent_shas", "omitted_parent_count", "truncated"]),
  "github.pull_request_get": object("Pull-request and changed-file evidence", { number: integer("Pull-request number"), title_preview: string("Bounded title", 500), state: string("Pull-request state", 32), draft: boolean("Draft state"), updated_at: string("Update timestamp", 64), head_sha: string("Head SHA", 40), base_sha: string("Base SHA", 40), changed_files: integer("Changed-file count"), labels: array("Labels", string("Label", 100)), omitted_label_count: integer("Omitted label count", 100), files: array("Changed-file summaries", file), truncated: boolean("Truncation state") }, ["number", "title_preview", "state", "draft", "updated_at", "head_sha", "base_sha", "changed_files", "labels", "omitted_label_count", "files", "truncated"]),
  "github.issue_get": object("Issue metadata evidence", { number: integer("Issue number"), title_preview: string("Bounded title", 500), body_preview: string("Bounded issue body", 1200), state: string("Issue state", 32), updated_at: string("Update timestamp", 64), author_login: string("Author login", 120), labels: array("Labels", string("Label", 100)), omitted_label_count: integer("Omitted label count", 100), truncated: boolean("Truncation state") }, ["number", "title_preview", "state", "updated_at", "author_login", "labels", "omitted_label_count", "truncated"]),
  "github.commit_checks": object("Exact-SHA check-run evidence", { commit_sha: string("Observed commit SHA", 40), observed_commit_sha: string("Observed exact commit SHA", 40), total_count: integer("Reported check count", 100_000), items: array("Check-run summaries", check), truncated: boolean("Truncation state") }, ["commit_sha", "observed_commit_sha", "total_count", "items", "truncated"]),
