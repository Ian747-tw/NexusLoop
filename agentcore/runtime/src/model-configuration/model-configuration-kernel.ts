...
export function validateCommanderModelConformanceRegistry(value: unknown): CommanderModelConformanceRegistry {
  ...
  for (let index = 0; index < rawEntries.length; index += 1) entries.push(parseConformanceEntry(rawEntries[index], index, policyVersion))
...
function parseConformanceEntry(value: unknown, index: number, policyVersion: typeof COMMANDER_MODEL_CONFORMANCE_POLICY_VERSION | typeof COMMANDER_MODEL_CONFORMANCE_POLICY_VERSION_V2): CommanderModelConformanceEntry {
  ...
  if (transportKind !== "google_generative_ai_connector" && providerKind === "google") fail("provider_kind google requires google_generative_ai_connector conformance")
  const stable = {
