...
export class ModelSetupService {
  ...
  async confirm(value: unknown): Promise<ModelSetupCommitResult> {
    ...
    if (projection.candidate?.candidate_hash === candidate.candidate_hash
      && projection.confirmed_by === input.confirmed_by
    ...
    try {
      await this.eventStore.appendIfLatestKind(event, MODEL_SETUP_EVENT_KIND, projection.latest_event_id)
      ...
      if (reconciled.revision === input.expected_revision + 1
        ...
        && reconciled.confirmed_by === input.confirmed_by) {
...
export function projectModelSetupEvents(events: readonly JsonlEvent[]): ModelSetupProjection {
  ...
  for (let index = 0; index < events.length; index += 1) {
    ...
    const canonicalConfirmedBy = setupOperatorIdentifier(input.confirmed_by)
    ...
    confirmedBy = canonicalConfirmedBy
    committedAt = canonicalCommittedAt
  }
  return deepFreeze(revision === 0
    ? { status: "missing", revision: 0, latest_event_id: null }
    : { status: "ready", revision, latest_event_id: latestEventId, setup_hash: setupHash, candidate, confirmed_by: confirmedBy, committed_at: committedAt })
}
