...
export class CommanderContinuityService {
  ...
  async midMission(input: CommanderMidMissionContinuityInput = {}): Promise<CommanderMidMissionContinuityPacket> {
    ...
    const sourceRefs = uniqueRefs([
      ...
      wakePreview ? ref("wake_supervisor", wakePreview.preview_id, "wake supervisor preview", `${wakePreview.supervisor_status}/${wakePreview.recommended_action}`, wakePreview.status) : undefined,
      wakeExecution ? ref("wake_supervisor_execution", wakeExecution.execution_id, wakeExecution.supervisor_status ?? "wake execution", wakeExecution.recommended_action ?? wakeExecution.execution_mode, wakeExecution.action_execution_status) : undefined,
      wakeAction ? ref("wake_action_execution", wakeAction.action_execution_id, wakeAction.action_kind, wakeAction.metadata_result_preview ?? wakeAction.manual_action_preview ?? wakeAction.recommended_action, wakeAction.status) : undefined,
    ...
    const sections = buildSections([
      ...
      section("wake_result_state", "Wake/result state", `wake=${wakePreview?.supervisor_status ?? "none"}; wake_execution=${wakeExecution?.recommended_action ?? "none"}; wake_action=${wakeAction?.status ?? "none"}; result_reports=${reports.length}; reviews=${reviews.length}; ingestions=${ingestions.length}`, sourceRefs.filter((item) => ["wake_supervisor", "wake_supervisor_execution", "wake_action_execution", "result_report", "result_review", "research_ingestion"].includes(item.source_kind)), (wakePreview ? 1 : 0) + (wakeExecution ? 1 : 0) + (wakeAction ? 1 : 0) + reports.length + reviews.length + ingestions.length),
    ...
    return redactContinuityPacket<CommanderMidMissionContinuityPacket>({
      ...
      wake_supervision_summary: [
        wakePreview ? `${wakePreview.supervisor_status}/${wakePreview.recommended_action}` : "wake supervisor preview unavailable",
        wakeExecution ? `execution=${wakeExecution.execution_id}/${wakeExecution.action_execution_status}` : "execution=none",
        wakeAction ? `action=${wakeAction.action_execution_id}/${wakeAction.status}` : "action=none",
      ].join("; "),
