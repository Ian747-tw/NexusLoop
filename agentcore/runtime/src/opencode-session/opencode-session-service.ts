...
export class OpenCodeSessionService {
  ...
  private async buildPreview(input: OpenCodeSessionPreviewInput): Promise<OpenCodeSessionPreview> {
    ...
    if (sourceKind === "unknown") blockers.push("unknown source_kind cannot create planned sessions")
    ...
    const terminalSourceStatus = source.source_statuses.find((status) => {
      if (status === "applied" && sourceKind === "executor_review" && source.apply_id) return false
      return ["cancelled", "rejected", "failed", "completed", "applied"].includes(status)
    })
