...
function recommendedCommands(reportId: string, sessionId: string | undefined, decision: OpenCodeResultReviewDecision, nextStep: OpenCodeResultReviewNextStep): OpenCodeResultReviewCommand[] {
  ...
  if (nextStep === "prepare_research_ingestion") commands.push({ label: "Prepare research ingestion", command: "/research-ingestion-preview report=<report_id>", command_type: "write", notes: "future branch only; manual explicit command required and not executed by 9O" })
