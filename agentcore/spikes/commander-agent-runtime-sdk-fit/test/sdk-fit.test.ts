...
describe("isolated SDK spike", () => {
  ...
  for (const adapter of adapters) {
    ...
    test(`${adapter.candidate_id} supports JSON fallback validation`, () => {
      ...
      const mixedToolAndFinal = runJsonFallbackProbe(request, JSON.stringify({ type: "tool_call", tool_id: "memory.search", arguments: { query: "memory" }, final: { summary: "conflicting" } }))
      expect(mixedToolAndFinal.status).toBe("malformed")
      ...
      const missingFinal = runJsonFallbackProbe(request, JSON.stringify({ type: "final" }))
      expect(missingFinal.status).toBe("malformed")
      const invalidFinal = runJsonFallbackProbe(request, JSON.stringify({ type: "final", final: { extra: "ignored" } }))
      expect(invalidFinal.status).toBe("malformed")
      const mixedFinalAndTool = runJsonFallbackProbe(request, JSON.stringify({ type: "final", final: { summary: "bounded final answer" }, tool_id: "memory.search" }))
      expect(mixedFinalAndTool.status).toBe("malformed")
      const validFinal = runJsonFallbackProbe(request, JSON.stringify({ type: "final", final: { summary: "bounded final answer" } }))
      expect(validFinal.status).toBe("final")
