...
describe("TUI keyboard command model", () => {
  ...
  test("dot and colon prefixed text remains a user message", () => {
    for (const message of [".status notes", ":missions", ".topics", ":research", ".mission", ":claim", ".complete", ":complete", ".reviews", ":approve", ".proposals", ":apply-proposal", ".bundles", ":apply-bundle", ".playbooks", ":draft-fail", ".drafts", ":draft-review", "/tmp/repro", "/path"]) {
      const state: UiState = {
        ...initialState("/tmp/demo"),
        screen: "main",
        focus: "message-box",
