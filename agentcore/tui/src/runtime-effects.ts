...
import type {
  ...
  OpenCodeResultReportCommandSummary,
  OpenCodeResultReportPreviewSummary,
  OpenCodeResultReportRecordSummary,
  ...
  CommanderGuidanceDeliveryCommandSummary,
  CommanderGuidanceDeliveryPreviewSummary,
  CommanderGuidanceDeliveryRecordSummary,
  ...
  OpenCodeWakeSupervisorBatchPreviewSummary,
  OpenCodeWakeSupervisorBatchResultSummary,
  OpenCodeWakeSupervisorExecutionCommandSummary,
  OpenCodeWakeSupervisorExecutionEvidenceRefSummary,
  OpenCodeWakeSupervisorExecutionPreviewSummary,
  OpenCodeWakeSupervisorExecutionRecordSummary,
...
export type RuntimeUiEffect =
  ...
  | { type: "preview-opencode-wake-supervisor-execution"; sessionId?: string; launchId?: string; includeResearchMemory?: boolean; includeContextPacket?: boolean; includeHumanControls?: boolean; includeGuidanceDelivery?: boolean; limitEvidence?: number }
  | { type: "record-opencode-wake-supervisor-execution"; sessionId?: string; launchId?: string; includeResearchMemory?: boolean; includeContextPacket?: boolean; includeHumanControls?: boolean; includeGuidanceDelivery?: boolean; limitEvidence?: number; dryRun?: boolean }
  | { type: "preview-opencode-wake-supervisor-batch"; limit?: number; statusFilter?: string; includeResearchMemory?: boolean; includeHumanControls?: boolean; includeGuidanceDelivery?: boolean; limitEvidence?: number }
  | { type: "record-opencode-wake-supervisor-batch"; limit?: number; statusFilter?: string; includeResearchMemory?: boolean; includeHumanControls?: boolean; includeGuidanceDelivery?: boolean; limitEvidence?: number; dryRun?: boolean }
  | { type: "load-opencode-wake-supervisor-executions"; sessionId?: string; launchId?: string; supervisorStatus?: string; recommendedAction?: string; executionMode?: string; limit?: number }
  ...
  | { type: "preview-opencode-result-report"; sessionId?: string; launchId?: string; resultKind?: string; summary?: string; outcome?: string; changedFiles?: string[]; testsRun?: string[]; testResults?: string[]; artifacts?: string[]; metrics?: string[]; claims?: string[]; knownFailures?: string[]; followups?: string[]; confidence?: string; progressId?: string; watchdogId?: string; questionId?: string; guidanceId?: string; deliveryId?: string; wakeExecutionId?: string; wakeActionExecutionId?: string; reviewState?: string }
  | { type: "record-opencode-result-report"; sessionId?: string; launchId?: string; resultKind?: string; summary?: string; outcome?: string; changedFiles?: string[]; testsRun?: string[]; testResults?: string[]; artifacts?: string[]; metrics?: string[]; claims?: string[]; knownFailures?: string[]; followups?: string[]; confidence?: string; progressId?: string; watchdogId?: string; questionId?: string; guidanceId?: string; deliveryId?: string; wakeExecutionId?: string; wakeActionExecutionId?: string; reviewState?: string; dryRun?: boolean }
  | { type: "load-opencode-result-reports"; sessionId?: string; launchId?: string; resultKind?: string; resultDisposition?: string; reviewState?: string; limit?: number }
  ...
  try {
    switch (effect.type) {
      ...
      case "deliver-commander-guidance": {
        ...
        let updated = next
        if (effect.dryRun !== true && result?.guidance_id) {
          updated = applyCommanderGuidanceSelected(updated, await runtime.command("runtime.get_commander_guidance", { guidanceId: result.guidance_id }), result.guidance_id)
          updated = applyCommanderGuidanceRecords(updated, await runtime.command("runtime.list_commander_guidance", { limit: HANDOFF_LIMIT, sessionId: result.session_id, launchId: result.launch_id }))
        }
        return effect.dryRun === true ? next : applyCommanderGuidanceDeliveryRecords(updated, await runtime.command("runtime.list_commander_guidance_deliveries", { limit: HANDOFF_LIMIT, guidanceId: result?.guidance_id ?? effect.guidanceId, sessionId: result?.session_id, launchId: result?.launch_id }))
      }
      case "load-commander-guidance-deliveries":
        return applyCommanderGuidanceDeliveryRecords(state, await runtime.command("runtime.list_commander_guidance_deliveries", { limit: effect.limit ?? HANDOFF_LIMIT, sessionId: effect.sessionId, launchId: effect.launchId, guidanceId: effect.guidanceId, status: effect.status, deliveryMode: effect.deliveryMode }))
      case "load-commander-guidance-delivery":
        return applyCommanderGuidanceDeliverySelected(state, await runtime.command("runtime.get_commander_guidance_delivery", { deliveryId: effect.deliveryId }), effect.deliveryId)
      case "load-latest-commander-guidance-delivery":
      ...
      case "preview-opencode-wake-supervisor-execution":
        return applyOpenCodeWakeSupervisorExecutionPreview(state, await runtime.command("runtime.preview_opencode_wake_supervisor_execution", opencodeWakeSupervisorExecutionPayload(effect)))
      case "record-opencode-wake-supervisor-execution": {
        const next = applyOpenCodeWakeSupervisorExecutionResult(state, await runtime.command("runtime.record_opencode_wake_supervisor_execution", { ...opencodeWakeSupervisorExecutionPayload(effect), dryRun: effect.dryRun === true, recordedBy: "operator" }))
        if (next.opencodeWakeSupervisorExecutions?.commandError || effect.dryRun === true) return next
        const result = next.opencodeWakeSupervisorExecutions?.latestResult
        return applyOpenCodeWakeSupervisorExecutionRecords(next, await runtime.command("runtime.list_opencode_wake_supervisor_executions", { limit: HANDOFF_LIMIT, sessionId: result?.session_id ?? effect.sessionId, launchId: result?.launch_id ?? effect.launchId }))
      }
      case "preview-opencode-wake-supervisor-batch":
        return applyOpenCodeWakeSupervisorBatchPreview(state, await runtime.command("runtime.preview_opencode_wake_supervisor_batch", opencodeWakeSupervisorBatchPayload(effect)))
      case "record-opencode-wake-supervisor-batch": {
        const next = applyOpenCodeWakeSupervisorBatchResult(state, await runtime.command("runtime.record_opencode_wake_supervisor_batch", { ...opencodeWakeSupervisorBatchPayload(effect), dryRun: effect.dryRun === true, recordedBy: "operator" }))
        if (next.opencodeWakeSupervisorExecutions?.commandError || effect.dryRun === true) return next
        return applyOpenCodeWakeSupervisorExecutionRecords(next, await runtime.command("runtime.list_opencode_wake_supervisor_executions", { limit: HANDOFF_LIMIT }))
      }
      case "load-opencode-wake-supervisor-executions":
        return applyOpenCodeWakeSupervisorExecutionRecords(state, await runtime.command("runtime.list_opencode_wake_supervisor_executions", { limit: effect.limit ?? HANDOFF_LIMIT, sessionId: effect.sessionId, launchId: effect.launchId, supervisorStatus: effect.supervisorStatus, recommendedAction: effect.recommendedAction, executionMode: effect.executionMode }))
      case "load-opencode-wake-supervisor-execution":
        return applyOpenCodeWakeSupervisorExecutionSelected(state, await runtime.command("runtime.get_opencode_wake_supervisor_execution", { executionId: effect.executionId }), effect.executionId)
      case "load-latest-opencode-wake-supervisor-execution":
      ...
      case "preview-opencode-result-report":
        return applyOpenCodeResultReportPreview(state, await runtime.command("runtime.preview_opencode_result_report", opencodeResultReportPayload(effect)))
      case "record-opencode-result-report": {
        const next = applyOpenCodeResultReportResult(state, await runtime.command("runtime.record_opencode_result_report", { ...opencodeResultReportPayload(effect), dryRun: effect.dryRun === true, recordedBy: "operator" }))
        if (next.opencodeResultReports?.commandError || effect.dryRun === true) return next
        const result = next.opencodeResultReports?.latestResult
        return applyOpenCodeResultReportRecords(next, await runtime.command("runtime.list_opencode_result_reports", { limit: HANDOFF_LIMIT, sessionId: result?.session_id ?? effect.sessionId, launchId: result?.launch_id ?? effect.launchId }))
      }
      case "load-opencode-result-reports":
        return applyOpenCodeResultReportRecords(state, await runtime.command("runtime.list_opencode_result_reports", { limit: effect.limit ?? HANDOFF_LIMIT, sessionId: effect.sessionId, launchId: effect.launchId, resultKind: effect.resultKind, resultDisposition: effect.resultDisposition, reviewState: effect.reviewState }))
      case "load-opencode-result-report":
        return applyOpenCodeResultReportSelected(state, await runtime.command("runtime.get_opencode_result_report", { reportId: effect.reportId }), effect.reportId)
      case "load-latest-opencode-result-report":
...
function applyCommanderGuidanceDeliveryPreview(state: UiState, value: unknown): UiState {
  const delivery = readCommanderGuidanceDeliveryPreview(value)
  const commandError = delivery.status === "blocked" ? delivery.blockers[0] ?? "Commander guidance delivery preview is blocked" : undefined
  return {
    ...state,
    commanderGuidanceDelivery: {
      ...commanderGuidanceDeliveryState(state),
      preview: delivery,
      commandError,
    },
    systemActions: [...state.systemActions, { title: "commander guidance delivery preview", detail: `guidance=${delivery.guidance_id || "missing"} status=${delivery.status}`, status: delivery.status }].slice(-12),
  }
}

function applyCommanderGuidanceDeliveryResult(state: UiState, value: unknown): UiState {
  const result = readCommanderGuidanceDeliveryResult(value)
  const commandError = result.status === "blocked" || result.status === "delivery_failed" ? result.error ?? "Commander guidance delivery is blocked or failed" : undefined
  return {
    ...state,
    commanderGuidanceDelivery: {
      ...commanderGuidanceDeliveryState(state),
      latestResult: result,
      latest: result.status === "delivery_requested" || result.status === "delivered" ? result : commanderGuidanceDeliveryState(state).latest ?? null,
      commandError,
    },
    systemActions: [...state.systemActions, { title: "commander guidance delivery", detail: `status=${result.status} delivery=${result.delivery_id} guidance=${result.guidance_id || "missing"}`, status: result.status }].slice(-12),
  }
}

function applyCommanderGuidanceDeliveryRecords(state: UiState, value: unknown): UiState {
  const records = readCommanderGuidanceDeliveryRecords(value)
  return {
    ...state,
    commanderGuidanceDelivery: {
      ...commanderGuidanceDeliveryState(state),
    ...
    systemActions: [...state.systemActions, { title: "commander guidance delivery records", detail: `records=${records.length}`, status: "loaded" }].slice(-12),
  }
}

function applyCommanderGuidanceDeliverySelected(state: UiState, value: unknown, deliveryId: string): UiState {
  const selected = value === null ? null : readCommanderGuidanceDeliveryResult(value)
  return {
    ...state,
    commanderGuidanceDelivery: {
      ...commanderGuidanceDeliveryState(state),
      selected,
      commandError: selected ? undefined : `Commander guidance delivery not found: ${redactText(deliveryId)}`,
    },
    systemActions: [...state.systemActions, { title: "commander guidance delivery selected", detail: `delivery=${redactText(deliveryId)}`, status: selected ? "loaded" : "missing" }].slice(-12),
  }
}

function applyCommanderGuidanceDeliveryLatest(state: UiState, value: unknown, label: string): UiState {
  const latest = value === null ? null : readCommanderGuidanceDeliveryResult(value)
  return {
    ...state,
    commanderGuidanceDelivery: {
      ...commanderGuidanceDeliveryState(state),
      latest,
      commandError: latest ? undefined : `Commander guidance delivery latest not found: ${redactText(label)}`,
    },
    systemActions: [...state.systemActions, { title: "commander guidance delivery latest", detail: `target=${redactText(label)}`, status: latest ? "loaded" : "missing" }].slice(-12),
  }
}

function applyCommanderGuidanceDeliverySummary(state: UiState, value: unknown): UiState {
  const summary = readCommanderGuidanceDeliverySummary(value)
  return {
    ...state,
    commanderGuidanceDelivery: {
      ...commanderGuidanceDeliveryState(state),
    ...
    systemActions: [...state.systemActions, { title: "commander guidance delivery summary", detail: `requested=${summary.requested_count} delivered=${summary.delivered_count}`, status: "loaded" }].slice(-12),
  }
}
...
function applyOpenCodeWakeSupervisorExecutionPreview(state: UiState, value: unknown): UiState {
  const preview = readOpenCodeWakeSupervisorExecutionPreview(value)
  const commandError = preview.status === "blocked" ? preview.blockers[0] ?? "OpenCode wake supervisor execution preview is blocked" : undefined
  return {
    ...state,
    opencodeWakeSupervisorExecutions: {
      ...opencodeWakeSupervisorExecutionState(state),
      preview,
      commandError,
    },
    systemActions: [...state.systemActions, { title: "opencode wake execution preview", detail: `session=${preview.session_id ?? "missing"} status=${preview.supervisor_status ?? "unknown"} action=${preview.recommended_action ?? "unknown"}`, status: preview.status }].slice(-12),
  }
}

function applyOpenCodeWakeSupervisorExecutionResult(state: UiState, value: unknown): UiState {
  const result = readOpenCodeWakeSupervisorExecutionResult(value)
  const commandError = result.status === "blocked" || result.status === "failed" ? result.error ?? "OpenCode wake supervisor execution is blocked" : undefined
  return {
    ...state,
    opencodeWakeSupervisorExecutions: {
      ...opencodeWakeSupervisorExecutionState(state),
      latestResult: result,
      latest: result.status === "recorded" ? result : opencodeWakeSupervisorExecutionState(state).latest,
      commandError,
    },
    systemActions: [...state.systemActions, { title: "opencode wake execution", detail: `execution=${result.execution_id} status=${result.status} action_execution_status=${result.action_execution_status}`, status: result.status }].slice(-12),
  }
}

function applyOpenCodeWakeSupervisorBatchPreview(state: UiState, value: unknown): UiState {
  const preview = readOpenCodeWakeSupervisorBatchPreview(value)
  const commandError = preview.status === "blocked" ? preview.blockers[0] ?? "OpenCode wake supervisor batch preview is blocked" : undefined
  return {
    ...state,
    opencodeWakeSupervisorExecutions: {
      ...opencodeWakeSupervisorExecutionState(state),
    ...
    systemActions: [...state.systemActions, { title: "opencode wake batch preview", detail: `included=${preview.included_session_count} skipped=${preview.skipped_session_count}`, status: preview.status }].slice(-12),
  }
}

function applyOpenCodeWakeSupervisorBatchResult(state: UiState, value: unknown): UiState {
  const result = readOpenCodeWakeSupervisorBatchResult(value)
  const commandError = result.status === "blocked" || result.status === "failed" ? result.error ?? "OpenCode wake supervisor batch execution is blocked" : undefined
  return {
    ...state,
    opencodeWakeSupervisorExecutions: {
      ...opencodeWakeSupervisorExecutionState(state),
      batchResult: result,
      records: result.status === "recorded" ? mergeExecutionRecords(opencodeWakeSupervisorExecutionState(state).records, result.execution_records) : opencodeWakeSupervisorExecutionState(state).records,
      commandError,
    },
    systemActions: [...state.systemActions, { title: "opencode wake batch execution", detail: `batch=${result.batch_id} recorded=${result.recorded_execution_count} action_execution_status=${result.action_execution_status}`, status: result.status }].slice(-12),
  }
}

function applyOpenCodeWakeSupervisorExecutionRecords(state: UiState, value: unknown): UiState {
  const records = readOpenCodeWakeSupervisorExecutionRecords(value)
  return {
    ...state,
    opencodeWakeSupervisorExecutions: {
      ...opencodeWakeSupervisorExecutionState(state),
    ...
    systemActions: [...state.systemActions, { title: "opencode wake executions", detail: `records=${records.length}`, status: "loaded" }].slice(-12),
  }
}

function applyOpenCodeWakeSupervisorExecutionSelected(state: UiState, value: unknown, executionId: string): UiState {
  const selected = value ? readOpenCodeWakeSupervisorExecutionResult(value) : null
  const commandError = selected ? undefined : `OpenCode wake supervisor execution not found: ${executionId}`
  return {
    ...state,
    opencodeWakeSupervisorExecutions: {
      ...opencodeWakeSupervisorExecutionState(state),
    ...
    systemActions: [...state.systemActions, { title: "opencode wake execution show", detail: selected ? `execution=${selected.execution_id}` : `missing=${executionId}`, status: selected ? "loaded" : "blocked" }].slice(-12),
  }
}

function applyOpenCodeWakeSupervisorExecutionLatest(state: UiState, value: unknown, target: string): UiState {
  const latest = value ? readOpenCodeWakeSupervisorExecutionResult(value) : null
  const commandError = latest ? undefined : `OpenCode wake supervisor latest execution not found: ${target}`
  return {
    ...state,
    opencodeWakeSupervisorExecutions: {
      ...opencodeWakeSupervisorExecutionState(state),
    ...
    systemActions: [...state.systemActions, { title: "opencode wake execution latest", detail: latest ? `execution=${latest.execution_id}` : `missing=${target}`, status: latest ? "loaded" : "blocked" }].slice(-12),
  }
}

function applyOpenCodeWakeSupervisorExecutionSummary(state: UiState, value: unknown): UiState {
  const summary = readOpenCodeWakeSupervisorExecutionSummary(value)
  return {
    ...state,
    opencodeWakeSupervisorExecutions: {
      ...opencodeWakeSupervisorExecutionState(state),
      summary,
      records: summary.latest_executions,
      commandError: undefined,
    },
    systemActions: [...state.systemActions, { title: "opencode wake execution summary", detail: `executions=${summary.total_executions} batches=${summary.batch_count} action_executed=${summary.action_executed_count}`, status: "loaded" }].slice(-12),
  }
}
...
function applyOpenCodeResultReportPreview(state: UiState, value: unknown): UiState {
  const preview = readOpenCodeResultReportPreview(value)
  const commandError = preview.status === "blocked" ? preview.blockers[0] ?? "OpenCode result report preview is blocked" : undefined
  return {
    ...state,
    opencodeResultReports: {
      ...opencodeResultReportState(state),
      preview,
      commandError,
    },
    systemActions: [...state.systemActions, { title: "opencode result report preview", detail: `session=${preview.session_id || "missing"} kind=${preview.result_kind} review=${preview.review_state}`, status: preview.status }].slice(-12),
  }
}

function applyOpenCodeResultReportResult(state: UiState, value: unknown): UiState {
  const result = readOpenCodeResultReportResult(value)
  const commandError = result.status === "blocked" || result.status === "failed" ? result.error ?? "OpenCode result report is blocked" : undefined
  return {
    ...state,
    opencodeResultReports: {
      ...opencodeResultReportState(state),
      latestResult: result,
      latest: result.status === "recorded" ? result : opencodeResultReportState(state).latest,
      commandError,
    },
    systemActions: [...state.systemActions, { title: "opencode result report", detail: `report=${result.report_id} status=${result.status} kind=${result.result_kind}`, status: result.status }].slice(-12),
  }
}

function applyOpenCodeResultReportRecords(state: UiState, value: unknown): UiState {
  const records = readOpenCodeResultReportRecords(value)
  return {
    ...state,
    opencodeResultReports: {
      ...opencodeResultReportState(state),
    ...
    systemActions: [...state.systemActions, { title: "opencode result reports", detail: `records=${records.length}`, status: "loaded" }].slice(-12),
  }
}

function applyOpenCodeResultReportSelected(state: UiState, value: unknown, reportId: string): UiState {
  const selected = value ? readOpenCodeResultReportResult(value) : null
  const commandError = selected ? undefined : `OpenCode result report not found: ${reportId}`
  return {
    ...state,
    opencodeResultReports: {
      ...opencodeResultReportState(state),
    ...
    systemActions: [...state.systemActions, { title: "opencode result report show", detail: selected ? `report=${selected.report_id}` : `missing=${reportId}`, status: selected ? "loaded" : "blocked" }].slice(-12),
  }
}

function applyOpenCodeResultReportLatest(state: UiState, value: unknown, target: string): UiState {
  const latest = value ? readOpenCodeResultReportResult(value) : null
  const commandError = latest ? undefined : `OpenCode result report latest not found: ${target}`
  return {
    ...state,
    opencodeResultReports: {
      ...opencodeResultReportState(state),
    ...
    systemActions: [...state.systemActions, { title: "opencode result report latest", detail: latest ? `report=${latest.report_id}` : `missing=${target}`, status: latest ? "loaded" : "blocked" }].slice(-12),
  }
}

function applyOpenCodeResultReportSummary(state: UiState, value: unknown): UiState {
  const summary = readOpenCodeResultReportSummary(value)
  return {
    ...state,
    opencodeResultReports: {
      ...opencodeResultReportState(state),
    ...
    systemActions: [...state.systemActions, { title: "opencode result report summary", detail: `reports=${summary.total_reports} review=${summary.needs_commander_review_count}`, status: "loaded" }].slice(-12),
  }
}
...
function applyNamedRuntimeCommand(state: UiState, runtime: RuntimeClient, command: string, args: string[]): Promise<UiState> {
  ...
  switch (command) {
    ...
    case "commander-guidance-delivery-preview":
    case "guidance-delivery-preview":
      return applyRuntimeUiEffect(commandState, runtime, commanderGuidanceDeliveryEffect("preview-commander-guidance-delivery", args, true))
    case "commander-guidance-delivery-dry-run":
    case "guidance-delivery-dry-run": {
      const effect = commanderGuidanceDeliveryEffect("deliver-commander-guidance", args, true) as Extract<RuntimeUiEffect, { type: "deliver-commander-guidance" }>
      return applyRuntimeUiEffect(commandState, runtime, { ...effect, dryRun: true })
    }
    case "commander-guidance-deliver":
    case "deliver-guidance":
    case "send-guidance":
      return applyRuntimeUiEffect(commandState, runtime, commanderGuidanceDeliveryEffect("deliver-commander-guidance", args, true))
    case "commander-guidance-deliveries":
    case "guidance-deliveries":
      return applyRuntimeUiEffect(commandState, runtime, commanderGuidanceDeliveryListEffect(args))
    case "commander-guidance-delivery-latest":
    case "guidance-delivery-latest":
      return applyRuntimeUiEffect(commandState, runtime, commanderGuidanceDeliveryLatestEffect(args))
    case "commander-guidance-delivery-show":
    ...
    case "opencode-wake-execution-preview":
    case "wake-execution-preview":
      return applyRuntimeUiEffect(commandState, runtime, opencodeWakeSupervisorExecutionEffect("preview-opencode-wake-supervisor-execution", args, false))
    case "opencode-wake-execution-dry-run":
    case "wake-execution-dry-run": {
      const effect = opencodeWakeSupervisorExecutionEffect("record-opencode-wake-supervisor-execution", args, true) as Extract<RuntimeUiEffect, { type: "record-opencode-wake-supervisor-execution" }>
      return applyRuntimeUiEffect(commandState, runtime, { ...effect, dryRun: true })
    }
    case "opencode-wake-execution-record":
    case "wake-execution-record":
      return applyRuntimeUiEffect(commandState, runtime, opencodeWakeSupervisorExecutionEffect("record-opencode-wake-supervisor-execution", args, true))
    case "opencode-wake-batch-preview":
    case "wake-batch-preview":
      return applyRuntimeUiEffect(commandState, runtime, opencodeWakeSupervisorBatchEffect("preview-opencode-wake-supervisor-batch", args, false))
    case "opencode-wake-batch-dry-run": {
      const effect = opencodeWakeSupervisorBatchEffect("record-opencode-wake-supervisor-batch", args, true) as Extract<RuntimeUiEffect, { type: "record-opencode-wake-supervisor-batch" }>
      return applyRuntimeUiEffect(commandState, runtime, { ...effect, dryRun: true })
    }
    case "wake-batch-dry-run": {
      const effect = opencodeWakeSupervisorBatchEffect("record-opencode-wake-supervisor-batch", args, true) as Extract<RuntimeUiEffect, { type: "record-opencode-wake-supervisor-batch" }>
      return applyRuntimeUiEffect(commandState, runtime, { ...effect, dryRun: true })
    }
    case "opencode-wake-batch-record":
    case "wake-batch-record":
      return applyRuntimeUiEffect(commandState, runtime, opencodeWakeSupervisorBatchEffect("record-opencode-wake-supervisor-batch", args, true))
    case "opencode-wake-executions":
    case "wake-executions":
      return applyRuntimeUiEffect(commandState, runtime, opencodeWakeSupervisorExecutionListEffect(args))
    case "opencode-wake-execution-latest":
    case "wake-execution-latest":
      return applyRuntimeUiEffect(commandState, runtime, opencodeWakeSupervisorExecutionLatestEffect(args))
    case "opencode-wake-execution-show":
      return applyRuntimeUiEffect(commandState, runtime, { type: "load-opencode-wake-supervisor-execution", executionId: requiredArg(args, 0, "executionId") })
    ...
    case "opencode-result-report-preview":
    case "result-report-preview":
      return applyRuntimeUiEffect(commandState, runtime, opencodeResultReportEffect("preview-opencode-result-report", args, false))
    case "opencode-result-report-dry-run":
    case "result-report-dry-run": {
      const effect = opencodeResultReportEffect("record-opencode-result-report", args, true) as Extract<RuntimeUiEffect, { type: "record-opencode-result-report" }>
      return applyRuntimeUiEffect(commandState, runtime, { ...effect, dryRun: true })
    }
    case "opencode-result-report":
    case "result-report":
      return applyRuntimeUiEffect(commandState, runtime, opencodeResultReportEffect("record-opencode-result-report", args, true))
    case "opencode-result-reports":
    case "result-reports":
      return applyRuntimeUiEffect(commandState, runtime, opencodeResultReportListEffect(args))
    case "opencode-result-report-latest":
    case "result-report-latest":
      return applyRuntimeUiEffect(commandState, runtime, opencodeResultReportLatestEffect(args))
    case "opencode-result-report-show":
      return applyRuntimeUiEffect(commandState, runtime, { type: "load-opencode-result-report", reportId: requiredArg(args, 0, "reportId") })
...
const commanderGuidanceDeliveryCommands = new Set([
...
const opencodeWakeSupervisorExecutionCommands = new Set([
...
const opencodeResultReportCommands = new Set([
...
function readCommanderGuidanceDeliveryPreview(value: unknown): CommanderGuidanceDeliveryPreviewSummary {
  if (!isRecord(value) || typeof value.preview_id !== "string") throw new Error("runtime.preview_commander_guidance_delivery returned invalid preview")
  return {
    preview_id: redactText(value.preview_id),
    status: readString(value.status, "blocked"),
    can_deliver: value.can_deliver === true,
    guidance_id: redactText(readString(value.guidance_id, "")),
    question_id: redactText(readString(value.question_id, "")),
    session_id: redactText(readString(value.session_id, "")),
    launch_id: typeof value.launch_id === "string" ? redactText(value.launch_id) : undefined,
    guidance_status: typeof value.guidance_status === "string" ? readString(value.guidance_status, "") : undefined,
    current_delivery_status: typeof value.current_delivery_status === "string" ? readString(value.current_delivery_status, "") : undefined,
    delivery_mode: readString(value.delivery_mode, "operator_handoff"),
    delivery_payload_preview: preview(readString(value.delivery_payload_preview, "")),
    answer_preview: preview(readString(value.answer_preview, "")),
    constraints_preview: readStringList(value.constraints_preview, 10).map(preview),
    rationale_preview: typeof value.rationale_preview === "string" ? preview(readString(value.rationale_preview, "")) : undefined,
    refs_preview: readStringList(value.refs_preview, 16).map(preview),
    target_summary_preview: preview(readString(value.target_summary_preview, "")),
    adapter_capability: readString(value.adapter_capability, "operator_handoff_only"),
    blockers: readStringList(value.blockers, 12).map(preview),
    warnings: readStringList(value.warnings, 14).map(preview),
    recommended_commands: readCommanderGuidanceDeliveryCommands(value.recommended_commands),
    generated_at: readString(value.generated_at, ""),
    redacted_summary_preview: preview(readString(value.redacted_summary_preview, "")),
    delivery_hash: readString(value.delivery_hash, ""),
  }
}

function readCommanderGuidanceDeliveryResult(value: unknown): CommanderGuidanceDeliveryResultSummary {
  if (!isRecord(value) || typeof value.delivery_id !== "string") throw new Error("runtime.deliver_commander_guidance returned invalid result")
  return {
    delivery_id: redactText(value.delivery_id),
    status: readString(value.status, "blocked"),
    guidance_id: redactText(readString(value.guidance_id, "")),
    question_id: redactText(readString(value.question_id, "")),
    session_id: redactText(readString(value.session_id, "")),
    launch_id: typeof value.launch_id === "string" ? redactText(value.launch_id) : undefined,
    delivery_mode: readString(value.delivery_mode, "operator_handoff"),
    delivery_status_after: readString(value.delivery_status_after, "not_delivered"),
    adapter_capability: readString(value.adapter_capability, "operator_handoff_only"),
    delivery_payload_preview: preview(readString(value.delivery_payload_preview, "")),
    target_summary_preview: preview(readString(value.target_summary_preview, "")),
    adapter_ack_preview: typeof value.adapter_ack_preview === "string" ? preview(readString(value.adapter_ack_preview, "")) : undefined,
    operator_handoff_preview: typeof value.operator_handoff_preview === "string" ? preview(readString(value.operator_handoff_preview, "")) : undefined,
    created_at: readString(value.created_at, ""),
    delivered_by: preview(readString(value.delivered_by, "")),
    error: typeof value.error === "string" ? preview(readString(value.error, "")) : undefined,
    delivery_hash: readString(value.delivery_hash, ""),
    recommended_commands: readCommanderGuidanceDeliveryCommands(value.recommended_commands),
  }
...
function readCommanderGuidanceDeliveryRecords(value: unknown): CommanderGuidanceDeliveryRecordSummary[] {
  if (!Array.isArray(value)) return []
  return value.filter(isRecord).slice(0, 20).map((record) => ({
    delivery_id: redactText(readString(record.delivery_id, "")),
    status: readString(record.status, "delivery_requested"),
    guidance_id: redactText(readString(record.guidance_id, "")),
    question_id: redactText(readString(record.question_id, "")),
    session_id: redactText(readString(record.session_id, "")),
    launch_id: typeof record.launch_id === "string" ? redactText(record.launch_id) : undefined,
    delivery_mode: readString(record.delivery_mode, "operator_handoff"),
    delivery_status_after: readString(record.delivery_status_after, "pending_delivery"),
    created_at: readString(record.created_at, ""),
    delivered_by: preview(readString(record.delivered_by, "")),
    summary_preview: preview(readString(record.summary_preview, "")),
    delivery_hash: readString(record.delivery_hash, ""),
  }))
}

function readCommanderGuidanceDeliverySummary(value: unknown): CommanderGuidanceDeliverySummaryState {
  if (!isRecord(value)) throw new Error("runtime.commander_guidance_delivery_summary returned invalid summary")
  return {
    total_deliveries: readNumber(value.total_deliveries, 0),
    requested_count: readNumber(value.requested_count, 0),
    delivered_count: readNumber(value.delivered_count, 0),
    failed_count: readNumber(value.failed_count, 0),
    by_mode_counts: readNumberMap(value.by_mode_counts, 12),
    latest_deliveries: readCommanderGuidanceDeliveryRecords(value.latest_deliveries),
    generated_at: readString(value.generated_at, ""),
  }
}
...
function readOpenCodeWakeSupervisorSummary(value: unknown): OpenCodeWakeSupervisorSummaryState {
  ...
  return {
    total_launched_sessions: readNumber(value.total_launched_sessions, 0),
    status_counts: readNumberMap(value.status_counts, 20),
    healthy_count: readNumber(value.healthy_count, 0),
    watch_count: readNumber(value.watch_count, 0),
    stale_count: readNumber(value.stale_count, 0),
    timed_out_count: readNumber(value.timed_out_count, 0),
    needs_report_count: readNumber(value.needs_report_count, 0),
...
function readOpenCodeWakeSupervisorExecutionPreview(value: unknown): OpenCodeWakeSupervisorExecutionPreviewSummary {
  if (!isRecord(value) || typeof value.preview_id !== "string") throw new Error("runtime.preview_opencode_wake_supervisor_execution returned invalid preview")
  return {
    preview_id: redactText(value.preview_id),
    status: readString(value.status, "blocked"),
    can_record: value.can_record === true,
    execution_mode: readString(value.execution_mode, "single_session"),
    session_id: typeof value.session_id === "string" ? redactText(value.session_id) : undefined,
    launch_id: typeof value.launch_id === "string" ? redactText(value.launch_id) : undefined,
    supervisor_preview_id: typeof value.supervisor_preview_id === "string" ? redactText(value.supervisor_preview_id) : undefined,
    supervisor_hash: typeof value.supervisor_hash === "string" ? readString(value.supervisor_hash, "") : undefined,
    ...
    action_execution_status: readString(value.action_execution_status, "not_executed"),
    recommended_commands_preview: readOpenCodeWakeSupervisorExecutionCommands(value.recommended_commands_preview),
    evidence_refs: readOpenCodeWakeSupervisorExecutionEvidenceRefs(value.evidence_refs),
    context_section_count: readNumber(value.context_section_count, 0),
    blockers: readStringList(value.blockers, 12).map(preview),
    warnings: readStringList(value.warnings, 14).map(preview),
    generated_at: readString(value.generated_at, ""),
    redacted_summary_preview: preview(readString(value.redacted_summary_preview, "")),
    execution_hash: readString(value.execution_hash, ""),
  }
}

function readOpenCodeWakeSupervisorExecutionResult(value: unknown): OpenCodeWakeSupervisorExecutionResultSummary {
  if (!isRecord(value) || typeof value.execution_id !== "string") throw new Error("runtime.record_opencode_wake_supervisor_execution returned invalid result")
  return {
    execution_id: redactText(value.execution_id),
    status: readString(value.status, "blocked"),
    execution_mode: readString(value.execution_mode, "single_session"),
    session_id: typeof value.session_id === "string" ? redactText(value.session_id) : undefined,
    launch_id: typeof value.launch_id === "string" ? redactText(value.launch_id) : undefined,
    ...
    error: typeof value.error === "string" ? preview(readString(value.error, "")) : undefined,
    execution_hash: readString(value.execution_hash, ""),
    recommended_commands: readOpenCodeWakeSupervisorExecutionCommands(value.recommended_commands),
  }
}

function readOpenCodeWakeSupervisorBatchPreview(value: unknown): OpenCodeWakeSupervisorBatchPreviewSummary {
  if (!isRecord(value) || typeof value.preview_id !== "string") throw new Error("runtime.preview_opencode_wake_supervisor_batch returned invalid preview")
  return {
    preview_id: redactText(value.preview_id),
    status: readString(value.status, "blocked"),
    can_record: value.can_record === true,
    execution_mode: readString(value.execution_mode, "batch_active_sessions"),
    total_candidate_sessions: readNumber(value.total_candidate_sessions, 0),
    included_session_count: readNumber(value.included_session_count, 0),
    skipped_session_count: readNumber(value.skipped_session_count, 0),
    session_previews: readOpenCodeWakeSupervisorExecutionPreviews(value.session_previews),
    blockers: readStringList(value.blockers, 12).map(preview),
    warnings: readStringList(value.warnings, 14).map(preview),
    generated_at: readString(value.generated_at, ""),
    redacted_summary_preview: preview(readString(value.redacted_summary_preview, "")),
...
function readOpenCodeWakeSupervisorBatchResult(value: unknown): OpenCodeWakeSupervisorBatchResultSummary {
  if (!isRecord(value) || typeof value.batch_id !== "string") throw new Error("runtime.record_opencode_wake_supervisor_batch returned invalid result")
  return {
    batch_id: redactText(value.batch_id),
    status: readString(value.status, "blocked"),
    execution_mode: readString(value.execution_mode, "batch_active_sessions"),
    total_candidate_sessions: readNumber(value.total_candidate_sessions, 0),
    recorded_execution_count: readNumber(value.recorded_execution_count, 0),
    skipped_session_count: readNumber(value.skipped_session_count, 0),
    execution_records: readOpenCodeWakeSupervisorExecutionRecords(value.execution_records),
    action_execution_status: readString(value.action_execution_status, "not_executed"),
    recorded_at: readString(value.recorded_at, ""),
    recorded_by: preview(readString(value.recorded_by, "")),
    error: typeof value.error === "string" ? preview(readString(value.error, "")) : undefined,
    batch_hash: readString(value.batch_hash, ""),
    recommended_commands: readOpenCodeWakeSupervisorExecutionCommands(value.recommended_commands),
  }
}

function readOpenCodeWakeSupervisorExecutionPreviews(value: unknown): OpenCodeWakeSupervisorExecutionPreviewSummary[] {
  if (!Array.isArray(value)) return []
  return value.filter(isRecord).slice(0, 20).map(readOpenCodeWakeSupervisorExecutionPreview)
}

function readOpenCodeWakeSupervisorExecutionRecords(value: unknown): OpenCodeWakeSupervisorExecutionRecordSummary[] {
  if (!Array.isArray(value)) return []
  return value.filter(isRecord).slice(0, 20).map((record) => ({
    execution_id: redactText(readString(record.execution_id, "")),
    execution_mode: readString(record.execution_mode, "single_session"),
    session_id: typeof record.session_id === "string" ? redactText(record.session_id) : undefined,
    launch_id: typeof record.launch_id === "string" ? redactText(record.launch_id) : undefined,
    supervisor_status: typeof record.supervisor_status === "string" ? readString(record.supervisor_status, "unknown") : undefined,
    recommended_action: typeof record.recommended_action === "string" ? readString(record.recommended_action, "unknown") : undefined,
    action_execution_status: readString(record.action_execution_status, "not_executed"),
    recorded_at: readString(record.recorded_at, ""),
    recorded_by: preview(readString(record.recorded_by, "")),
    summary_preview: preview(readString(record.summary_preview, "")),
    execution_hash: readString(record.execution_hash, ""),
  }))
}

function readOpenCodeWakeSupervisorExecutionSummary(value: unknown): OpenCodeWakeSupervisorExecutionSummaryState {
  if (!isRecord(value)) throw new Error("runtime.opencode_wake_supervisor_execution_summary returned invalid summary")
  return {
    total_executions: readNumber(value.total_executions, 0),
    session_count: readNumber(value.session_count, 0),
    batch_count: readNumber(value.batch_count, 0),
    healthy_count: readNumber(value.healthy_count, 0),
    watch_count: readNumber(value.watch_count, 0),
    needs_report_count: readNumber(value.needs_report_count, 0),
    needs_commander_answer_count: readNumber(value.needs_commander_answer_count, 0),
    ...
    stale_count: readNumber(value.stale_count, 0),
    blocked_count: readNumber(value.blocked_count, 0),
    action_executed_count: readNumber(value.action_executed_count, 0),
    latest_executions: readOpenCodeWakeSupervisorExecutionRecords(value.latest_executions),
    generated_at: readString(value.generated_at, ""),
  }
}

function readOpenCodeWakeSupervisorExecutionEvidenceRefs(value: unknown): OpenCodeWakeSupervisorExecutionEvidenceRefSummary[] {
  if (!Array.isArray(value)) return []
  return value.filter(isRecord).slice(0, 20).map((ref) => ({
    evidence_kind: readString(ref.evidence_kind, "unknown"),
    evidence_id: redactText(readString(ref.evidence_id, "")),
...
function readOpenCodeWakeSupervisorExecutionCommands(value: unknown): OpenCodeWakeSupervisorExecutionCommandSummary[] {
  if (!Array.isArray(value)) return []
  return value.filter(isRecord).slice(0, 12).map((command) => ({
    label: preview(readString(command.label, "")),
    command: preview(readString(command.command, "")),
...
function readOpenCodeResultReportPreview(value: unknown): OpenCodeResultReportPreviewSummary {
  ...
  return {
    ...
    session_id: redactText(readString(value.session_id, "")),
    launch_id: typeof value.launch_id === "string" ? redactText(value.launch_id) : undefined,
    result_kind: readString(value.result_kind, "unknown"),
    result_disposition: readString(value.result_disposition, "reported_status_only"),
    review_state: readString(value.review_state, "unknown"),
    summary_preview: preview(readString(value.summary_preview, "")),
    outcome_preview: typeof value.outcome_preview === "string" ? preview(readString(value.outcome_preview, "")) : undefined,
    changed_files_preview: readStringList(value.changed_files_preview, 12).map(preview),
    tests_run_preview: readStringList(value.tests_run_preview, 12).map(preview),
    test_results_preview: readStringList(value.test_results_preview, 12).map(preview),
    artifacts_preview: readStringList(value.artifacts_preview, 12).map(preview),
    ...
    known_failures_preview: readStringList(value.known_failures_preview, 12).map(preview),
    ...
    commander_review_created: value.commander_review_created === true,
    blockers: readStringList(value.blockers, 12).map(preview),
    warnings: readStringList(value.warnings, 14).map(preview),
    recommended_commands: readOpenCodeResultReportCommands(value.recommended_commands),
    generated_at: readString(value.generated_at, ""),
    redacted_summary_preview: preview(readString(value.redacted_summary_preview, "")),
    report_hash: readString(value.report_hash, ""),
  }
}

function readOpenCodeResultReportResult(value: unknown): OpenCodeResultReportResultSummary {
  if (!isRecord(value) || typeof value.report_id !== "string") throw new Error("runtime.record_opencode_result_report returned invalid result")
  return {
    report_id: redactText(value.report_id),
    status: readString(value.status, "blocked"),
    session_id: redactText(readString(value.session_id, "")),
    launch_id: typeof value.launch_id === "string" ? redactText(value.launch_id) : undefined,
    result_kind: readString(value.result_kind, "unknown"),
...
function readOpenCodeResultReportRecords(value: unknown): OpenCodeResultReportRecordSummary[] {
  if (!Array.isArray(value)) return []
  return value.filter(isRecord).slice(0, 20).map((record) => ({
    report_id: redactText(readString(record.report_id, "")),
    session_id: redactText(readString(record.session_id, "")),
    launch_id: typeof record.launch_id === "string" ? redactText(record.launch_id) : undefined,
    result_kind: readString(record.result_kind, "unknown"),
    result_disposition: readString(record.result_disposition, "reported_status_only"),
    review_state: readString(record.review_state, "unknown"),
    summary_preview: preview(readString(record.summary_preview, "")),
    recorded_at: readString(record.recorded_at, ""),
    recorded_by: preview(readString(record.recorded_by, "")),
    confidence: readPrimitiveConfidence(record.confidence),
    has_failures: record.has_failures === true,
    has_artifacts: record.has_artifacts === true,
    has_metrics: record.has_metrics === true,
    report_hash: readString(record.report_hash, ""),
  }))
}

function readOpenCodeResultReportSummary(value: unknown): OpenCodeResultReportSummaryState {
  if (!isRecord(value)) throw new Error("runtime.opencode_result_report_summary returned invalid summary")
  return {
    total_reports: readNumber(value.total_reports, 0),
    session_count: readNumber(value.session_count, 0),
    completion_count: readNumber(value.completion_count, 0),
    partial_count: readNumber(value.partial_count, 0),
    failure_count: readNumber(value.failure_count, 0),
    inconclusive_count: readNumber(value.inconclusive_count, 0),
    blocked_count: readNumber(value.blocked_count, 0),
    needs_commander_review_count: readNumber(value.needs_commander_review_count, 0),
    needs_human_review_count: readNumber(value.needs_human_review_count, 0),
    latest_reports: readOpenCodeResultReportRecords(value.latest_reports),
    generated_at: readString(value.generated_at, ""),
  }
}
...
function readPrimitiveConfidence(value: unknown): number | string | undefined {
  if (typeof value === "number" && Number.isFinite(value)) return value
  if (typeof value === "string") return preview(readString(value, ""))
  return undefined
}
...
function mergeExecutionRecords(existing: OpenCodeWakeSupervisorExecutionRecordSummary[], incoming: OpenCodeWakeSupervisorExecutionRecordSummary[]): OpenCodeWakeSupervisorExecutionRecordSummary[] {
  ...
  for (const record of [...incoming, ...existing]) {
    if (seen.has(record.execution_id)) continue
...
function commanderGuidanceDeliveryState(state: UiState): CommanderGuidanceDeliveryState {
...
function opencodeWakeSupervisorExecutionState(state: UiState): OpenCodeWakeSupervisorExecutionsState {
...
function opencodeResultReportState(state: UiState): OpenCodeResultReportState {
...
function commanderGuidanceDeliveryPayload(effect: Extract<RuntimeUiEffect, { type: "preview-commander-guidance-delivery" | "deliver-commander-guidance" }>): Record<string, unknown> {
  return {
...
function opencodeWakeSupervisorExecutionPayload(effect: Extract<RuntimeUiEffect, { type: "preview-opencode-wake-supervisor-execution" | "record-opencode-wake-supervisor-execution" }>): Record<string, unknown> {
  return {
    sessionId: effect.sessionId,
    launchId: effect.launchId,
    includeResearchMemory: effect.includeResearchMemory,
...
function opencodeWakeSupervisorBatchPayload(effect: Extract<RuntimeUiEffect, { type: "preview-opencode-wake-supervisor-batch" | "record-opencode-wake-supervisor-batch" }>): Record<string, unknown> {
  return {
    limit: effect.limit,
    statusFilter: effect.statusFilter,
    includeResearchMemory: effect.includeResearchMemory,
...
function opencodeResultReportPayload(effect: Extract<RuntimeUiEffect, { type: "preview-opencode-result-report" | "record-opencode-result-report" }>): Record<string, unknown> {
  return {
    sessionId: effect.sessionId,
    launchId: effect.launchId,
    ...
    testsRun: effect.testsRun,
    testResults: effect.testResults,
    artifacts: effect.artifacts,
    metrics: effect.metrics,
    claims: effect.claims,
    knownFailures: effect.knownFailures,
...
function commanderGuidanceDeliveryEffect(
  type: "preview-commander-guidance-delivery" | "deliver-commander-guidance",
  args: string[],
  requirePayload: boolean,
): Extract<RuntimeUiEffect, { type: "preview-commander-guidance-delivery" | "deliver-commander-guidance" }> {
  const effect: Extract<RuntimeUiEffect, { type: "preview-commander-guidance-delivery" | "deliver-commander-guidance" }> = { type }
  const knownKeys = new Set(["guidance", "mode", "allow_real_delivery", "operator_note", "delivered_by"])
  for (let index = 0; index < args.length; index += 1) {
    const { key, value, nextIndex } = readKeyValueWithFreeText(args, index, knownKeys, new Set(["operator_note"]), "Commander guidance delivery args must use guidance=<id>, mode=<mode>, allow_real_delivery=<bool>, operator_note=<text>, or delivered_by=<id>")
    index = nextIndex
    if (key === "guidance") effect.guidanceId = value
...
function commanderGuidanceDeliveryListEffect(args: string[]): Extract<RuntimeUiEffect, { type: "load-commander-guidance-deliveries" }> {
  const effect: Extract<RuntimeUiEffect, { type: "load-commander-guidance-deliveries" }> = { type: "load-commander-guidance-deliveries", limit: HANDOFF_LIMIT }
  for (const arg of args) {
    const [key, ...rest] = arg.split("=")
    const value = rest.join("=").trim()
    if (!key || !value) throw new Error("Commander guidance delivery list args must use guidance=<id>, session=<id>, launch=<id>, status=<status>, mode=<mode>, or limit=<n>")
    if (key === "guidance") effect.guidanceId = value
    else if (key === "session") effect.sessionId = value
    else if (key === "launch") effect.launchId = value
    else if (key === "status") effect.status = value
    ...
    else throw new Error("Commander guidance delivery list arg is unsupported")
  }
  return effect
}

function commanderGuidanceDeliveryLatestEffect(args: string[]): Extract<RuntimeUiEffect, { type: "load-latest-commander-guidance-delivery" }> {
  const effect: Extract<RuntimeUiEffect, { type: "load-latest-commander-guidance-delivery" }> = { type: "load-latest-commander-guidance-delivery" }
  for (const arg of args) {
    const [key, ...rest] = arg.split("=")
    const value = rest.join("=").trim()
    if (!key || !value) throw new Error("Commander guidance delivery latest args must use guidance=<id>, session=<id>, or launch=<id>")
    if (key === "guidance") effect.guidanceId = value
    else if (key === "session") effect.sessionId = value
    else if (key === "launch") effect.launchId = value
...
function opencodeWakeSupervisorExecutionEffect(
  type: "preview-opencode-wake-supervisor-execution" | "record-opencode-wake-supervisor-execution",
  args: string[],
  requireTarget: boolean,
): Extract<RuntimeUiEffect, { type: "preview-opencode-wake-supervisor-execution" | "record-opencode-wake-supervisor-execution" }> {
  const effect: Extract<RuntimeUiEffect, { type: "preview-opencode-wake-supervisor-execution" | "record-opencode-wake-supervisor-execution" }> = { type }
  for (const arg of args) {
    const [key, ...rest] = arg.split("=")
    const value = rest.join("=").trim()
    if (!key || !value) throw new Error("OpenCode wake execution args must use session=<id>, launch=<id>, include_research_memory=<bool>, include_context_packet=<bool>, include_human_controls=<bool>, include_guidance_delivery=<bool>, or limit=<n>")
    if (key === "session") effect.sessionId = value
    else if (key === "launch") effect.launchId = value
    else if (key === "include_research_memory") effect.includeResearchMemory = readBooleanArg(value)
    else if (key === "include_context_packet") effect.includeContextPacket = readBooleanArg(value)
    ...
    else throw new Error("OpenCode wake execution arg is unsupported")
  }
  if (requireTarget && !effect.sessionId && !effect.launchId) throw new Error("OpenCode wake execution requires session=<id> or launch=<id>")
  return effect
}

function opencodeWakeSupervisorBatchEffect(
  type: "preview-opencode-wake-supervisor-batch" | "record-opencode-wake-supervisor-batch",
  args: string[],
  _canRunWithoutTarget: boolean,
): Extract<RuntimeUiEffect, { type: "preview-opencode-wake-supervisor-batch" | "record-opencode-wake-supervisor-batch" }> {
  const effect: Extract<RuntimeUiEffect, { type: "preview-opencode-wake-supervisor-batch" | "record-opencode-wake-supervisor-batch" }> = { type, limit: HANDOFF_LIMIT }
  for (const arg of args) {
    const [key, ...rest] = arg.split("=")
    const value = rest.join("=").trim()
    if (!key || !value) throw new Error("OpenCode wake batch args must use status=<status>, include_research_memory=<bool>, include_human_controls=<bool>, include_guidance_delivery=<bool>, limit_evidence=<n>, or limit=<n>")
    if (key === "status") effect.statusFilter = value
    else if (key === "include_research_memory") effect.includeResearchMemory = readBooleanArg(value)
    else if (key === "include_human_controls") effect.includeHumanControls = readBooleanArg(value)
    else if (key === "include_guidance_delivery") effect.includeGuidanceDelivery = readBooleanArg(value)
    else if (key === "limit_evidence") effect.limitEvidence = readPositiveInteger(value, "limit_evidence", HANDOFF_LIMIT)
    else if (key === "limit") effect.limit = readPositiveInteger(value, "limit", HANDOFF_LIMIT)
    else throw new Error("OpenCode wake batch arg is unsupported")
  }
  return effect
}

function opencodeWakeSupervisorExecutionListEffect(args: string[]): Extract<RuntimeUiEffect, { type: "load-opencode-wake-supervisor-executions" }> {
  const effect: Extract<RuntimeUiEffect, { type: "load-opencode-wake-supervisor-executions" }> = { type: "load-opencode-wake-supervisor-executions", limit: HANDOFF_LIMIT }
  for (const arg of args) {
    const [key, ...rest] = arg.split("=")
    const value = rest.join("=").trim()
    if (!key || !value) throw new Error("OpenCode wake executions args must use session=<id>, launch=<id>, status=<status>, action=<action>, mode=<mode>, or limit=<n>")
    if (key === "session") effect.sessionId = value
    else if (key === "launch") effect.launchId = value
    else if (key === "status") effect.supervisorStatus = value
    else if (key === "action") effect.recommendedAction = value
    else if (key === "mode") effect.executionMode = value
    else if (key === "limit") effect.limit = readPositiveInteger(value, "limit", HANDOFF_LIMIT)
    else throw new Error("OpenCode wake executions arg is unsupported")
  }
  return effect
}

function opencodeWakeSupervisorExecutionLatestEffect(args: string[]): Extract<RuntimeUiEffect, { type: "load-latest-opencode-wake-supervisor-execution" }> {
  const effect: Extract<RuntimeUiEffect, { type: "load-latest-opencode-wake-supervisor-execution" }> = { type: "load-latest-opencode-wake-supervisor-execution" }
  for (const arg of args) {
    const [key, ...rest] = arg.split("=")
    const value = rest.join("=").trim()
    if (!key || !value) throw new Error("OpenCode wake execution latest args must use session=<id> or launch=<id>")
    if (key === "session") effect.sessionId = value
    else if (key === "launch") effect.launchId = value
...
function opencodeResultReportEffect(
  type: "preview-opencode-result-report" | "record-opencode-result-report",
  args: string[],
  requireSummary: boolean,
): Extract<RuntimeUiEffect, { type: "preview-opencode-result-report" | "record-opencode-result-report" }> {
  const effect: Extract<RuntimeUiEffect, { type: "preview-opencode-result-report" | "record-opencode-result-report" }> = { type }
  const knownKeys = new Set(["session", "launch", "kind", "summary", "outcome", "changed_files", "tests_run", "test_results", "artifacts", "metrics", "claims", "known_failures", "followups", "confidence", "progress", "watchdog", "question", "guidance", "delivery", "wake_execution", "wake_action", "review_state", "recorded_by"])
  const freeTextKeys = new Set(["summary", "outcome"])
  for (let index = 0; index < args.length; index += 1) {
    const { key, value, nextIndex } = readKeyValueWithFreeText(args, index, knownKeys, freeTextKeys, "OpenCode result report args must use session=<id>, launch=<id>, kind=<kind>, summary=<text>, outcome=<text>, changed_files=<csv>, tests_run=<csv>, test_results=<csv>, artifacts=<csv>, metrics=<csv>, claims=<csv>, known_failures=<csv>, followups=<csv>, confidence=<value>, progress=<id>, watchdog=<id>, question=<id>, guidance=<id>, delivery=<id>, wake_execution=<id>, wake_action=<id>, or review_state=<state>")
    index = nextIndex
    if (key === "session") effect.sessionId = value
    else if (key === "launch") effect.launchId = value
    else if (key === "kind") effect.resultKind = value
    else if (key === "summary") effect.summary = value
    else if (key === "outcome") effect.outcome = value
    else if (key === "changed_files") effect.changedFiles = commaList(value)
    else if (key === "tests_run") effect.testsRun = commaList(value)
    else if (key === "test_results") effect.testResults = commaList(value)
    else if (key === "artifacts") effect.artifacts = commaList(value)
    else if (key === "metrics") effect.metrics = commaList(value)
    else if (key === "claims") effect.claims = commaList(value)
    else if (key === "known_failures") effect.knownFailures = commaList(value)
    else if (key === "followups") effect.followups = commaList(value)
    else if (key === "confidence") effect.confidence = value
    else if (key === "progress") effect.progressId = value
    else if (key === "watchdog") effect.watchdogId = value
    else if (key === "question") effect.questionId = value
    ...
    else if (key === "wake_execution") effect.wakeExecutionId = value
    else if (key === "wake_action") effect.wakeActionExecutionId = value
    else if (key === "review_state") effect.reviewState = value
    else if (key === "recorded_by") {
      // Accepted for forward compatibility; runtime records operator for this branch.
...
function opencodeResultReportListEffect(args: string[]): Extract<RuntimeUiEffect, { type: "load-opencode-result-reports" }> {
  const effect: Extract<RuntimeUiEffect, { type: "load-opencode-result-reports" }> = { type: "load-opencode-result-reports", limit: HANDOFF_LIMIT }
  for (const arg of args) {
    const [key, ...rest] = arg.split("=")
    const value = rest.join("=").trim()
    if (!key || !value) throw new Error("OpenCode result reports args must use session=<id>, launch=<id>, kind=<kind>, disposition=<disposition>, review_state=<state>, or limit=<n>")
    if (key === "session") effect.sessionId = value
    else if (key === "launch") effect.launchId = value
    else if (key === "kind") effect.resultKind = value
    else if (key === "disposition") effect.resultDisposition = value
    else if (key === "review_state") effect.reviewState = value
    else if (key === "limit") effect.limit = readPositiveInteger(value, "limit", HANDOFF_LIMIT)
    else throw new Error("OpenCode result reports arg is unsupported")
  }
  return effect
}

function opencodeResultReportLatestEffect(args: string[]): Extract<RuntimeUiEffect, { type: "load-latest-opencode-result-report" }> {
  const effect: Extract<RuntimeUiEffect, { type: "load-latest-opencode-result-report" }> = { type: "load-latest-opencode-result-report" }
  for (const arg of args) {
    const [key, ...rest] = arg.split("=")
    const value = rest.join("=").trim()
    if (!key || !value) throw new Error("OpenCode result report latest args must use session=<id> or launch=<id>")
    if (key === "session") effect.sessionId = value
    else if (key === "launch") effect.launchId = value
