...
Branch 9S upgrades that read path to lightweight hybrid retrieval. Structured filters run first; SQLite FTS5/BM25 over bounded accepted research-memory fields is used when available; the existing deterministic lexical scorer remains the fallback and reranker. The search profile now reports whether FTS is active or why it fell back, while semantic, vector, embedding, provider, MCP, and online research remain disabled. FTS index maintenance is projection/search infrastructure only and does not create research evidence, proposals, missions, checkpoints, follow-up missions, OpenCode prompts, or mission/proposal/review/apply mutations. The FTS projection is durable and incrementally synchronized: legacy or new-version indexes are backfilled transactionally once through the write-authorized projection repair path, accepted/rejected result changes update only the affected row, explicit `rebuildFromEvents` reconstructs FTS, and normal consistent `ResearchDb.open()` performs no full delete/reinsert rebuild. Missing or stale FTS membership is repaired through the internal projection repair path only when write authority is held; read-only opens with dirty FTS state report bounded lexical fallback instead of repairing. Session-scoped research-memory retrieval remains best-effort; if a source projection lacks session IDs and the bounded global memory preview is used, the response must warn that session-scoped memory is unavailable.
...
- long-term research memory from the 9S bounded hybrid FTS+lexical search, near-duplicate, inspection, and search-profile surfaces
...
9R appends no continuity events, writes no `research.db` rows, creates no Commander proposal or follow-up mission, calls no provider/MCP/online research, sends no OpenCode prompts, launches or controls no OpenCode process, creates no checkpoints, and mutates no mission/proposal/review/apply state. Future Commander proposal work consumes continuity packets through an explicit proposal gate after the tool-capability registry and investigation loop are in place.
...
Branch 9U defines the Commander tool capability registry and investigation contract. It clarifies that a single authoritative runtime does not imply one shared model session: Commander and OpenCode keep separate roles and context windows while runtime owns the ledger and write barriers. 9U publishes a curated allowlist of Commander capabilities, phase profiles, namespace summaries, deferred schema loading, bootstrap schema budgeting, and registry validation. It does not execute tools, call providers/MCPs/network, read repository or GitHub content, append events, generate proposals, mutate missions, write `research.db`, or control OpenCode.

9U treats fixed Commander context packets as recovery and authority kernels, not scripted research flows. A future Commander investigation loop will dynamically choose from bounded read tools inside a phase capability envelope. Tool output is evidence only and has `instruction_semantics=none`; repository, GitHub, external research, and tool-description content cannot alter NexusLoop policy or authority. Direct proposal creation is not registered as a Commander tool. The updated sequence is 9V internal reads, 9W investigation loop, 9X external read gateway, 9Y proposal gate, 9Z governance gate, and later OpenCode delivery/resume integration.
