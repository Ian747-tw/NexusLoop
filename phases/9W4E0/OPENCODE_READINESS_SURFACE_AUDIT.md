...
The command runs from the packaged binary, suppresses ordinary models.dev refresh and database migration, accepts no provider list request, returns no model list, and never constructs a language model. It mirrors `ModelsDev.Data` local precedence by reading the default OpenCode `models.json` cache before the bundled snapshot. The cache read is capped at 8 MiB and validated with the pure schema shared with `src/provider/models.ts`; malformed, oversized, unreadable, custom-path, or custom-URL catalog authority yields `unknown` without refresh or fallback to a false-complete claim.
...
The supported readiness build embeds `test/tool/fixtures/models-api.json` as an explicit local fallback input. At implementation time it is 2,408,942 bytes with SHA-256 `33f836f532fd8ada58f255f11030ff5500d45b0cf7e63587772221050ffb1f48`. No live catalog fetch is part of the build or readiness command. A valid bounded default cache can supersede that fallback exactly as it does during ordinary Executor startup.
