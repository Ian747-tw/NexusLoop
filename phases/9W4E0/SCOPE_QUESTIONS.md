...
11. **How are alias, package, and discovered-plugin authority handled?** A configured alias inherits its earlier catalog or configured target's status. Exact packaged provider IDs are code-owned; a selected package outside that set is dynamic and yields `unknown`. Only `.ts` and `.js` files, including symlinks, under OpenCode's `plugin` or `plugins` directories count as auto-discovered plugins. `OPENCODE_PURE` suppresses external plugin authority.
12. **How are legacy and overlapping configuration sources handled?** JSON/JSONC and nested-directory precedence matches `ConfigPaths`; deprecated TUI-only keys are normalized away before schema validation. The legacy global TOML source cannot be migrated in the read-only command, so its presence yields `unknown`.
