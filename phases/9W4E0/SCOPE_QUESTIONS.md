...
11. **How are alias, package, and discovered-plugin authority handled?** A configured alias inherits its catalog target's status. Exact packaged provider IDs are code-owned; a selected package outside that set is dynamic and yields `unknown`. Only `.ts` and `.js` files under OpenCode's `plugin` or `plugins` directories count as auto-discovered plugins.
