...
| Alternate readiness executable or source preload replaces authority | Runtime derives readiness from the same validated process-adapter command used for launch and supplies only the fixed packaged subcommand; no readiness executable/args/preload input exists. |
| OpenCode state produces unsafe or partial certainty | The packaged command owns pinned OpenCode semantics and emits bounded tri-state evidence; Runtime maps malformed, timed-out, cancelled, mismatched, or failed observations to unknown. |
