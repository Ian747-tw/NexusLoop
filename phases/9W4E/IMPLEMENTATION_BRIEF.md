...
5. Add a bounded process-isolated OpenCode-side Executor readiness observer.
   ...
   invoking a bounded local config projection, and rejects locally active
   organization accounts at the same boundary. It never calls the side-effectful
   OpenCode config service, so readiness observation cannot initiate remote
   well-known/account fetches or config-directory package installation.
