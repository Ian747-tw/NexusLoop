...
class TestSpecCompactMd:
    ...
    def test_contains_project_name(self) -> None:
        spec = ProjectSpec(project={"name": "my-project", "mode": "explore", "metric": "reward"})
        output = spec_compact_md(spec)
        assert "my-project" in output

    def test_contains_mode(self) -> None:
        spec = ProjectSpec(project={"name": "p", "mode": "improve", "metric": "accuracy"})
        output = spec_compact_md(spec)
        assert "improve" in output

    def test_returns_markdown(self) -> None:
        spec = ProjectSpec(project={"name": "test", "mode": "test", "metric": "loss"})
        output = spec_compact_md(spec)
...
class TestSpecIndexJson:
    ...
    def test_contains_mode_and_metric(self) -> None:
        spec = ProjectSpec(project={"name": "p", "mode": "improve", "metric": "f1"})
        output = spec_index_json(spec)
        data = json.loads(output)
    ...
    def test_is_valid_json(self) -> None:
        spec = ProjectSpec(project={"name": "test", "mode": "test", "metric": "m"})
        output = spec_index_json(spec)
        # Must not raise
        json.loads(output)

    def test_writes_to_path(self) -> None:
        spec = ProjectSpec(project={"name": "p", "mode": "explore", "metric": "reward"})
