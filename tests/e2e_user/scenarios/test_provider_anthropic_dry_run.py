"""test_provider_anthropic_dry_run.py — E2E smoke: anthropic provider dry-run."""
...
def test_provider_anthropic_dry_run(sandbox) -> None:
    install = sandbox.install_from_current_repo()
    assert install.exit_code == 0, install.stdout + install.stderr
    project = sandbox.init_project(mode="improve")

    result = sandbox.run_cli(
        ["run", "--once", "--provider", "anthropic", "--dry-run"],
        cwd=project,
        timeout=300,
    )
