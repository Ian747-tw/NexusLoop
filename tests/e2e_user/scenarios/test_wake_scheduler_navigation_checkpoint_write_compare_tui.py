...
def test_user_compares_checkpoint_write_runs_through_tui_without_execution(sandbox) -> None:
    ...
    project = sandbox.make_empty_project_dir("wake_scheduler_navigation_checkpoint_write_compare_project")
    spec_dir = project / ".nxl" / "spec"
    spec_dir.mkdir(parents=True)
    (spec_dir / "current.json").write_text(
        json.dumps(
            {
                "spec_id": "spec_wake_scheduler_navigation_checkpoint_write_compare_e2e",
                "version": 1,
                "status": "approved",
                "objective": "Exercise scheduler navigation checkpoint write comparison through real runtime TUI",
                "project_mode": "build",
                "domain": "test",
                "success_metrics": ["scheduler checkpoint write comparison renders"],
    ...
    keys = [
        ...
        {"type": "insert", "text": f"/scheduler-nav-checkpoint-run {checkpoint_staged_id}"},
        {"type": "submit"},
        {"type": "insert", "text": "/scheduler-nav-checkpoint-history"},
        {"type": "submit"},
        {"type": "insert", "text": f"/scheduler-nav-checkpoint-compare {checkpoint_staged_id}"},
        {"type": "submit"},
        {"type": "insert", "text": "/scheduler-nav-checkpoint-stale after=1d"},
        {"type": "submit"},
        {"type": "insert", "text": "/scheduler-nav-checkpoint-approval-usage"},
        {"type": "submit"},
        {"type": "insert", "text": f"/scheduler-nav-checkpoint-group {checkpoint_staged_id}"},
        {"type": "submit"},
    ]
    encoded_keys = json.dumps(keys)
    sandbox.env["NXL_TUI_KEYS"] = encoded_keys
    sandbox.runner.env["NXL_TUI_KEYS"] = encoded_keys
    sandbox.env["NXL_SECRET_CHECKPOINT_COMPARE_TOKEN"] = "navigation-checkpoint-compare-secret-abc123"
    sandbox.runner.env["NXL_SECRET_CHECKPOINT_COMPARE_TOKEN"] = "navigation-checkpoint-compare-secret-abc123"
    ...
    assert "scheduler_checkpoint_write_comparison" in result.stdout
    assert "history_groups=1 runs=2" in result.stdout
    assert "approval_usage total=" in result.stdout
    assert "comparison uses bounded summaries and separate checkpoint artifact hashes" in result.stdout
    ...
    assert "navigation-checkpoint-compare-secret" not in result.stdout
    assert "navigation-checkpoint-compare-secret-abc123" not in result.stdout
    ...
    assert event_kinds.count("runtime_wake_scheduler_navigation_checkpoint_write_run_started") == 2
    assert event_kinds.count("runtime_wake_scheduler_navigation_checkpoint_write_run_succeeded") == 2
    assert event_kinds.count("runtime_checkpoint_created") == 2
    ...
    assert [event["staged_write_id"] for event in checkpoint_run_events] == [
        checkpoint_staged_id,
        checkpoint_staged_id,
    ]
    assert {event["execution_kind"] for event in checkpoint_run_events} == {"checkpoint_create"}
    assert len({event["run_id"] for event in checkpoint_run_events}) == 2
    ...
    assert "navigation-checkpoint-compare-secret" not in serialized_events
    assert "navigation-checkpoint-compare-secret-abc123" not in serialized_events
