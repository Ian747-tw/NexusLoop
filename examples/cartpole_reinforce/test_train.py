...
different hyperparameters and writes all data in the format the NexusLoop
...
eval that should be promoted to registry/dashboard, or a raw-log-only test:
  ...
  • .nxl/state.json          — updated best metric + phase
...
from nxl.logging.registry import ExperimentRegistry
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Test training harness for NexusLoop dashboard.")
    ...
    print("=" * 60)
    print("  NexusLoop — Test Training Harness")
